...
Method: 基于PuzzleBoard模式的独特组合结构，开发圆柱形标记PuzzlePole，实现360度视角的识别和姿态估计。

Result: PuzzlePole在定位和定向方面提供高精度，同时对遮挡具有鲁棒性。

Conclusion: 该设计为自主系统场景提供了灵活的部署方案，适用于机器人导航、SLAM和可触摸界面等应用。
...
TL;DR: PIGReward是一个个性化的奖励模型，通过动态生成用户条件评估维度和CoT推理来评估文本到图像生成模型与个人偏好的对齐程度，无需用户特定训练即可实现个性化评估和反馈。
...
Method: 采用自引导策略，在有限参考数据上进行推理构建丰富的用户上下文，通过动态生成用户条件评估维度和CoT推理来评估图像，并提供个性化反馈以优化提示。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习视觉模型中训练点对测试预测影响的估计对于定位噪声数据至关重要，但现有方法存在计算成本高、训练非平稳性、缺乏置信度校准等问题。

Method: SG-OIF框架将算法稳定性作为实时控制器，维护轻量级锚点IHVP，提出模块化曲率后端，使用稳定性引导的残差阈值、异常门控和置信度来调节每个样本的影响分数。

Result: 在多个数据集和各种损坏情况下，SG-OIF在噪声标签和分布外检测任务上达到SOTA，在CIFAR-10(20%不对称噪声)上top 1%预测样本准确率达91.1%，在MNIST上AUPR得分达99.8%。

Conclusion: SG-OIF框架是用于在线影响估计的实用控制器，有效解决了现有方法的挑战。
...
TL;DR: Pistachio是一个新的视频异常检测(VAD)和视频异常理解(VAU)基准，通过生成式流水线构建，提供精确的场景控制、异常类型和时序叙事，解决了现有基准在场景多样性、异常覆盖平衡性和时序复杂性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有视频异常检测基准缺乏场景多样性、平衡的异常覆盖和时序复杂性，难以可靠评估真实世界性能。同时，视频异常理解需要更深层的语义和因果推理，但由于手动标注工作量大而难以建立基准。

Method: 利用视频生成模型的进展，通过场景条件异常分配、多步骤故事情节生成和时间一致的长格式合成策略，构建了完全通过生成式流水线创建的基准，生成41秒的连贯视频。

Result: 广泛的实验证明了Pistachio的规模、多样性和复杂性，揭示了现有方法面临的新挑战，并推动了对动态和多事件异常理解的未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常使用专门架构或特定模态参数处理跟踪和分割任务，限制了泛化性和可扩展性。不同模态间的分布差异和任务间的特征表示差距阻碍了跨任务和跨模态知识共享。
...
<details>
  <summary>Details</summary>
Motivation: 姿态估计是计算机视觉中的重要问题，现有方法如SVD和四元数特征系统方法能精确解决含噪声的3D-3D对齐问题，但对于含噪声的3D-2D正交投影任务尚无已知的闭式解。

Method: 使用行列式比值矩阵（DRaM）方法求解无误差EnP和OnP问题的最小二乘系统，并采用简单的旋转校正方案处理含噪声数据。

Result: 提出了EnP和OnP旋转估计问题的新解决方案，并将QR分解和Moore-Penrose伪逆变换等方法统一到DRaM家族框架中。

Conclusion: 这项工作不仅为3D和2D正交投影姿态估计问题提供了新解决方案，还深入洞察了这类问题的本质，证明DRaM方法可推广到所有类似的N维欧几里得姿态估计问题。
...
TL;DR: LatentDreamer是一个从单张图像生成3D物体的新框架，使用预训练的变分自编码器将3D几何映射到潜在特征，显著降低了3D生成的难度。该框架依次生成粗糙几何、精细几何和逼真纹理，生成过程快速（约70秒），且生成结果与输入图像高度一致。
...
Result: LatentDreamer生成的3D物体与输入图像高度一致，整个生成过程可在约70秒内完成。大量实验表明，即使仅使用少量训练数据，LatentDreamer也能与当代方法竞争。

Conclusion: LatentDreamer通过将3D几何映射到潜在特征，显著降低了3D生成的难度，实现了快速、高保真度的3D物体生成，具有与当代方法竞争的性能。
...
TL;DR: 本文研究了模型容量与保留图像语义所需最小视觉token数量之间的基本关系，提出了一种基于正交过滤的轻量级模块来压缩冗余token，并揭示了token与模型规模的缩放规律。


<details>
  <summary>Details</summary>
Motivation: 受最小描述长度原理启发，研究模型容量与图像语义保留所需最小视觉token数量的根本关系，重新解释视觉token为视觉语义空间中的向量。

Method: 提出正交过滤（Orthogonal Filtering）轻量级模块，自适应地将冗余token聚类为紧凑的正交基集合，并贡献了一个视觉长上下文数据集。

Result: 在多种ViT模型上的实验表明存在一致的token与模型缩放规律：更大的模型需要显著更少的token来跨越视觉语义空间。

Conclusion: 模型容量与所需视觉token数量之间存在明确的缩放关系，正交过滤方法能有效压缩冗余token，为视觉语义表示提供了新的理论基础。
...
TL;DR: GroundingAgent是一个无需任务特定微调的智能视觉定位框架，通过整合预训练的开词汇目标检测器、多模态大语言模型和大语言模型，采用结构化迭代推理机制来渐进式优化候选区域。
...
Conclusion: GroundingAgent展示了无需微调的视觉定位可行性，强调了LLM推理能力的关键作用，并提供强可解释性，透明展示每个推理步骤。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型规模不断增长带来部署和效率挑战，传统压缩方法缺乏理论保证，需要一种基于信息论原理的压缩框架。

Method: 提出基于信息瓶颈原则的压缩框架，引入熵基有效秩(eRank)和KS距离量化注意力头贡献，设计训练式注意力头剪枝和免训练FFN压缩两种方案。
...
TL;DR: 提出了一种新的眼睑角度(ELA)指标，用于检测驾驶员疲劳状态。ELA基于3D面部特征点，比传统的2D眼动指标更稳定，能够生成逼真的合成数据集用于训练。
...
Result: 在公开数据集上，ELA比EAR在视角变化下方差更低，实现准确眨眼检测，合成数据增强提高了疲劳识别训练数据的多样性。
...
TL;DR: 提出了Perceptual Taxonomy基准测试，用于评估基于物理的视觉推理能力，包含3173个对象的84种精细属性标注，5802张图像和28033个模板问题，发现现有视觉语言模型在属性驱动问题上表现下降10-20%。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言基准测试缺乏对结构化场景理解的全面评估，主要关注表面识别或图像文本对齐，而人类认知需要识别对象及其空间配置，然后推断任务相关属性以支持目标导向推理。

Method: 构建Perceptual Taxonomy基准测试，标注3173个对象的四个属性族（材料、功能、可用性、物理属性），创建5802张图像的多种选择题基准，包含28033个模板问题和50个专家设计问题。

Result: 领先的视觉语言模型在识别任务上表现良好，但在属性驱动问题上性能下降10-20%，特别是在需要结构化属性多步推理的问题上。提供模拟场景的上下文推理示例能提高真实世界和专家策划问题的性能。

Conclusion: 当前模型在结构化视觉理解方面存在持续差距，主要依赖模式匹配。感知分类法引导的提示方法能有效提升推理性能，揭示了现有模型在深层视觉推理能力上的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 视频已成为互联网主要通信和创作媒介，需要可扩展的高质量视频制作能力。现有模型在时空定位和视频问答方面存在局限，需要更全面的多模态推理能力。

Method: Vidi2模型通过端到端时空定位技术，能够根据文本查询识别对应时间戳和目标对象的边界框。同时引入VUE-STG新基准，提供长上下文推理、高质量标注和优化评估指标。

Result: Vidi2在VUE-TR-V2和VUE-STG基准测试中显著超越Gemini 3 Pro和GPT-5等专有系统，在视频问答基准上与类似规模的开源模型竞争性结果。

Conclusion: Vidi2展示了强大的端到端时空定位能力，为复杂编辑场景提供支持，新基准VUE-STG为实际应用中的时空定位评估提供了更全面的框架。
...
TL;DR: 本研究探讨了多模态大语言模型在全球光伏评估中的跨域泛化能力，通过结构化提示和微调，在未见区域实现了优于传统计算机视觉方法的性能。
...
Method: 采用多模态大语言模型，通过结构化提示和微调，将检测、定位和量化集成到统一框架中。

Result: 跨区域评估显示，所提模型在未见区域性能下降最小，优于传统CV和transformer基线方法。
...
TL;DR: 该论文提出了大规模分析制图遗产的方法和数据集，通过77万+地图记录和9.9万+数字化图像，结合语义分割和对象检测技术，揭示了地图作为文化对象反映政治和认识论期望的特征。


<details>
  <summary>Details</summary>
Motivation: 当前自动化地图分析技术很少与制图史结合，忽视了地图作为语义符号系统和文化对象的本质。本研究旨在从文化视角大规模研究制图遗产。

Method: 收集来自38个数字目录的771,561条地图记录和99,715张数字化图像，应用语义分割和对象检测模型进行土地类别和制图符号的通用识别，分析空间焦点、合作模式和符号传播。

Result: 揭示了制图空间焦点与政治动态的关联，记录了国家化趋势和军事冲突对出版量的影响，编码了6300万个符号和2500万个片段，发现了如晕渲法被等高线替代等图形演变规律。

Conclusion: 地图是经过设计的图像，其构图强调特定特征，制图符号形成局部一致系统，合法性和主要城市在符号规范和符号文化传播中发挥关键作用。
...
TL;DR: ThiFAN-VQA是一个用于灾后损害评估的两阶段推理视觉问答框架，通过思维链提示和上下文学习生成结构化推理轨迹，再通过答案选择模块评估响应，在无人机数据集上实现了优越的准确性、可解释性和适应性。


<details>
  <summary>Details</summary>
Motivation: 解决现有AI框架在灾后损害评估中数据标注成本高、数据集规模有限、传统分类方法灵活性不足以及生成模型产生幻觉或通用响应的问题。

Method: 提出两阶段推理框架：第一阶段使用思维链提示和上下文学习生成结构化推理轨迹；第二阶段通过答案选择模块评估生成的响应并选择最连贯和上下文准确的答案。结合定制信息检索系统、领域特定提示和推理引导的答案选择。

Result: 在FloodNet和RescueNet-VQA无人机数据集上的实验表明，ThiFAN-VQA在准确性、可解释性和适应性方面优于现有方法，适用于真实世界的灾后损害评估任务。

Conclusion: ThiFAN-VQA通过结合零样本和监督方法的优势，在灵活性和一致性之间取得了平衡，为灾后损害评估提供了一个有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统OCR专家模型功能单一和通用视觉语言模型效率低下的问题，通过轻量级框架统一多功能性，采用端到端架构消除预处理模块依赖，并探索高质量数据和强化学习策略在OCR任务中的关键作用。

Method: 采用原生视觉Transformer和轻量级LLM通过MLP适配器连接的架构，实现纯端到端范式，避免传统流程中的错误传播。通过高质量数据和强化学习策略提升性能。

Result: 在ICDAR 2025 DIMT挑战赛（小模型赛道）获得第一名，在OCRBench上达到最先进水平，性能超越商业API、传统流程和更大模型（如Qwen3-VL-4B）。

Conclusion: HunyuanOCR在多功能性与效率的统一、端到端架构简化以及数据驱动和强化学习策略方面取得突破，为前沿研究和工业应用提供了坚实基础，已在HuggingFace开源并提供高性能部署方案。
...
TL;DR: 本文提出了一种基于VAE的多尺度向量量化变分自编码器(MSVQ-VAE)方法，用于生成无线胶囊内窥镜(WCE)医学图像，特别是能够条件生成包含不同类型异常(如息肉、血管和炎症)的合成图像，以解决医学数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 解决无线胶囊内窥镜图像分析中由于隐私限制和标注成本导致的医学数据稀缺问题，为临床决策支持系统提供足够的训练数据。

Method: 开发了多尺度向量量化变分自编码器(MSVQ-VAE)，能够无缝地将异常引入正常WCE图像中，支持条件生成不同类型的异常图像。

Result: 实验表明，使用该方法生成的异常图像训练的CDS分类器与仅使用真实数据训练的分类器性能相当。

Conclusion: 该方法在WCE图像生成方面表现出色，其通用性使其可应用于各种医学多媒体领域，为解决医学数据稀缺问题提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 智能眼镜上的自我中心感知可以改变我们在物理世界中学习新技能的方式，但自动技能评估仍然是一个基本的技术挑战。需要开发能够高效评估技能的方法。

Method: 采用两阶段框架：首先学习联合建模注视和自我中心视频来预测技能水平，然后蒸馏出仅需注视输入的学生模型。推理时只需注视输入，消除了连续视频处理。

Result: 在烹饪、音乐和体育三个数据集上的实验首次确立了注视在不同现实场景中技能理解的重要作用。教师模型达到最先进性能，仅注视学生变体在保持高精度的同时功耗比竞争方法低73倍。

Conclusion: 这些结果为在野外环境中实现AI支持的技能学习铺平了道路，证明了注视数据在技能评估中的价值以及节能方法的可行性。
...
Method: 提出语义分布引导重建框架，使用预训练视觉语言基础模型编码重建图像和辅助信息为高级语义特征，采用对比目标对齐重建表示与目标语义分布，可与各种深度学习方法结合并灵活整合多模态语义先验。
...
<details>
  <summary>Details</summary>
Motivation: 现有通用大型多模态模型在医学图像解释特别是病理学千兆像素图像分析中表现不佳，先前研究使用低分辨率缩略图或随机补丁可能低估了模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个结合影像和临床数据的AI系统，以提高乳腺癌诊断的准确性，减少观察者间变异性，并为资源匮乏地区提供可扩展的筛查解决方案。

Method: 使用基于注意力的编码器-解码器架构分割四个ROI（肿块、钙化、腋窝发现和乳腺组织），预测十项临床特征，并开发两种晚期融合策略整合多模态数据。

Result: 系统实现了最先进的分割准确性，稳健地预测临床特征，通过多模态数据融合提高了诊断精度，并开发了用户友好的Web应用程序。

Conclusion: OncoVision通过结合准确分割和临床直觉，为基于AI的乳腺X光检查设定了新标准，提供了可扩展和公平的解决方案，能够在早期检测乳腺癌并通过及时干预改善治疗。
...
TL;DR: IndEgo是一个多模态工业任务数据集，包含3,460个第一人称视角和1,092个第三人称视角的录制视频，涵盖装配/拆卸、物流组织、检查维修等工业任务，特别关注协作工作场景。


<details>
  <summary>Details</summary>
Motivation: 为了解决工业环境中复杂任务理解的需求，特别是协作工作的认知和体力密集型任务，需要包含丰富多模态数据和详细标注的数据集来支持相关研究。

Method: 收集了约197小时的第一人称视角和97小时第三人称视角视频，包含眼动追踪、语音叙述、声音、运动等多种模态数据，并提供动作、摘要、错误标注、叙述等详细注释。

Result: 数据集提供了处理后的眼动、手部姿态、半密集点云等输出，并在程序性和非程序性任务理解、错误检测和推理问答等任务上建立了基准测试。

Conclusion: 基线评估表明该数据集对当前最先进的多模态模型具有挑战性，为工业任务理解研究提供了有价值的资源。
...
Conclusion: RADSeg实现了零样本开放词汇语义分割的最先进性能，同时显著降低了计算和内存成本。
...
TL;DR: 提出一种基于分支的结构重参数化技术，通过训练阶段的并行分支设计，在推理时合并为精简的单路径模型，将ViT-Tiny从12层压缩到3-6层，保持ImageNet-1K准确率的同时实现移动CPU上37%的推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有ViT加速策略主要关注算法级优化，但深度架构带来的计算开销问题未得到充分探索。研究能否在减少Transformer层数的同时保持相当的表示能力。

Method: 基于分支的结构重参数化技术，在训练阶段使用并行分支，通过逐步合并非线性组件入口处的分支，对FFN和MHSA模块进行精确数学重参数化，无推理时近似误差。

Result: 成功将ViT-Tiny从12层压缩到6、4甚至3层，在ImageNet-1K上保持分类准确率，在移动CPU平台上实现高达37%的推理加速。

Conclusion: 传统偏好极深Transformer堆叠的智慧可能过于保守，为构建高效视觉Transformer开辟了新机会，证明了在减少层数的同时保持模型性能的可行性。
...
TL;DR: 提出了一种基于无人机高度的动态分块方法，通过自适应图像分割来提升海上搜救任务中的小目标检测性能，在保持检测精度的同时显著提高推理速度。


<details>
  <summary>Details</summary>
Motivation: 无人机在海上搜救任务中具有重要作用，但由于高空拍摄导致目标像素比例低，小目标检测困难。需要一种能够适应不同飞行高度的高效检测方法。

Method: 采用高度感知的动态分块方法，结合高度依赖的缩放和自适应分块因子，动态调整图像分割策略，减少不必要的计算量。

Result: 在SeaDronesSee数据集上测试，相比基线方法，小目标的平均精度提升了38%，推理速度比静态分块方法快两倍以上。

Conclusion: 该方法能够显著提升无人机在海上搜救任务中的小目标检测效率和准确性，适用于多样化的工作条件。
...
TL;DR: 本文研究了切片最优传输计划(min-STP)框架中优化切片器的可转移性，证明了在数据分布轻微扰动下优化切片器保持稳定，并提出小批量min-STP以提高可扩展性。


<details>
  <summary>Details</summary>
Motivation: 切片传输方法通过利用一维OT问题的闭式解降低了计算成本，但现有方法未解决学习到的最优切片器是否能在分布偏移下转移到新分布对的问题，这在数据演化或重复OT计算的场景中至关重要。

Method: 研究min-STP框架，分析优化切片器的可转移性；引入minibatch min-STP公式化以提高可扩展性；提供统计精度保证；在点云对齐和基于流的生成建模中进行实证验证。

Result: 理论证明优化切片器在数据分布轻微扰动下保持接近；实证表明可转移min-STP在一次性匹配中表现优异，并促进点云对齐和流生成建模的摊销训练。

Conclusion: 优化的切片器具有跨相关任务的有效转移能力，min-STP框架为处理演化数据或重复OT计算提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 尽管计算病理学基础模型有前景，但由于全切片图像处理的复杂性、学习特征的模糊性以及广泛的适应策略，将其适应特定临床任务仍具挑战性。

Method: 开发PathFMTools工具包，用于与CONCH和MUSK两个最先进的视觉语言基础模型交互，并在440例皮肤鳞状细胞癌H&E全切片图像上对多种适应策略进行基准测试。
...
TL;DR: VESSA是一个将视觉语言模型集成到半监督医学图像分割中的框架，通过两阶段方法结合基础级视觉语义理解，在标注数据极少的情况下显著提升分割精度。


<details>
  <summary>Details</summary>
Motivation: 半监督学习可以减少对专家标注的依赖，而视觉语言模型在跨视觉领域具有强大的泛化和少样本能力，将两者结合可以提升医学图像分割性能。

Method: 两阶段方法：第一阶段训练VESSA作为参考引导的分割助手，使用包含黄金标准示例的模板库；第二阶段将VESSA集成到最先进的SSL框架中，实现与学生模型的动态交互。

Result: 在多个分割数据集和领域的广泛实验中，VESSA增强的SSL显著提高了分割精度，在标注极其有限的情况下优于最先进的基线方法。

Conclusion: VESSA成功将视觉语言模型的能力融入半监督医学图像分割，为标注数据稀缺的场景提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 点云损伤重建虽然能有效避免基于图像方法易受背景噪声影响的问题，但其应用受限于3D数据的高体积。需要一种能够压缩数据同时保持有效信息的特征。

Method: 提出相对角度特征，计算为点法向量与其父点云平均法向量之间的夹角。通过基于熵的特征评估来筛选冗余信息，并使用PointNet++模型进行训练和测试。

Result: 基于相对角度的模型实现了与基于法向量模型相似的性能，同时实现了27.6%的存储减少和83%的输入通道压缩。
...
TL;DR: CrispFormer通过三个关键改进增强SegFormer解码器：边界分支监督物体轮廓、不确定性引导的细化器处理噪声标签、动态多尺度融合层自适应选择特征尺度，显著提升了弱监督语义分割的性能。


<details>
  <summary>Details</summary>
Motivation: 弱监督语义分割需要从噪声和不完整的监督信号中学习密集掩码，现有方法在边界精度、小物体识别和噪声鲁棒性方面存在不足。

Method: 在SegFormer解码器中添加：(1)边界分支使用轻量级边缘头和边界感知损失监督物体轮廓；(2)不确定性引导细化器预测像素级不确定性，用于加权损失和残差校正；(3)动态多尺度融合层用空间softmax门控替代静态拼接，可选由不确定性调制。

Result: 在标准WSSS流程中，CrispFormer相比SegFormer基线在边界F分数、小物体召回率和mIoU上均有显著提升，同时计算开销增加很小。
...
Method: 采用Prune-Then-Plan框架，首先使用基于Holm-Bonferroni的剪枝程序去除不可行的边界选择，然后将最终决策委托给基于覆盖率的规划器，将过度自信的预测转化为保守、可解释的动作。

Result: 在3D-Mem EQA框架中集成该方法，在视觉基础SPL和LLM-Match指标上分别比基线相对提高了49%和33%，在OpenEQA和EXPRESS-Bench数据集上实现了更好的场景覆盖。

Conclusion: 该方法通过步骤级校准稳定了探索过程，将人类级别的判断用于校准VLM的步骤级行为，在同等探索预算下实现了更好的性能。
...
TL;DR: 本文发现扩散变换器在混合分辨率去噪中使用线性插值旋转位置编码时会出现核心故障模式，导致注意力机制崩溃。作者提出了跨分辨率相位对齐注意力(CRPA)作为无需训练的即插即用解决方案。


<details>
  <summary>Details</summary>
Motivation: 当不同空间网格的token混合时，线性坐标重映射导致注意力头比较不兼容采样率的RoPE相位，产生相位混叠，破坏分数景观。预训练的DiT对此特别脆弱，即使微小的跨分辨率不一致也会导致模糊、伪影或完全崩溃。

Method: 提出Cross-Resolution Phase-Aligned Attention (CRPA)，仅修改每个注意力调用的RoPE索引映射：所有Q/K位置都以查询的步长表示，确保相同物理距离始终产生相同的相位增量。

Result: CRPA恢复了DiT依赖的精确相位模式，与预训练DiT完全兼容，稳定所有头和层。在图像和视频生成中优于先前最先进方法，实现高保真和高效的混合分辨率生成。

Conclusion: CRPA从根本上消除了混合分辨率生成中的相位混叠问题，提供了一种无需训练即可稳定预训练DiT的有效解决方案。
...
TL;DR: 本文提出了一种名为潜在表示探测（LRP）的方法，通过训练轻量级探测器来分析VLM的内部隐藏状态或注意力模式，以改进在STVQA任务中的弃权能力，实验显示该方法比最佳基线提高了7.6%的弃权准确率。


<details>
  <summary>Details</summary>
Motivation: 随着VLM在安全关键应用中的部署，其在不确定时能够弃权回答的能力变得至关重要，特别是在场景文本视觉问答（STVQA）任务中。现有的弃权方法要么依赖未校准的输出概率，要么需要不适合OCR任务的语义一致性，这表明不确定性信号可能隐藏在VLM的内部表示中。

Method: 提出了潜在表示探测（LRP）方法，通过训练轻量级探测器来分析VLM的内部隐藏状态或注意力模式。探索了三种探测器设计：跨所有层连接表示、聚合视觉标记的注意力、以及通过多数投票集成单层探测器。

Result: 在四个基准测试（涵盖图像和视频模态）上的实验表明，LRP比最佳基线提高了7.6%的弃权准确率。分析显示：探测器能够泛化到各种不确定性来源和数据集，且最优信号出现在中间层而非最终层。
...
TL;DR: CropVLM是一种低成本的外部方法，通过强化学习训练，使视觉语言模型能够动态放大相关图像区域，提升细粒度图像理解能力，无需人工标注边界框或昂贵的合成评估。
...
Method: 使用强化学习训练CropVLM，无需人工标注的边界框作为监督信号，也不依赖昂贵的合成评估。该方法训练一次后可与开源和专有视觉语言模型配对使用。

Result: 在需要高分辨率图像理解的任务上取得了显著改进，特别是在目标视觉语言模型域外基准测试中表现突出。

Conclusion: CropVLM能够有效提升视觉语言模型的细粒度图像理解能力，无需修改或微调原始模型，从而避免灾难性遗忘问题。
...
TL;DR: ReDirector是一种新颖的相机控制视频重拍生成方法，通过改进RoPE的位置编码和引入Rotary Camera Encoding，能够处理动态捕获的变长视频，实现更好的相机控制和几何一致性。
...
Method: 提出Rotary Camera Encoding (RoCE)，这是一种相机条件的RoPE相位偏移，能够捕获和整合输入视频与目标视频内部和跨视频的多视角关系。通过将相机条件集成到RoPE中，方法能够泛化到分布外的相机轨迹和视频长度。

Result: 实验表明，该方法在各种轨迹和长度下显著提高了相机可控性、几何一致性和视频质量，改进了动态对象定位和静态背景保持。

Conclusion: ReDirector通过改进的位置编码和相机条件集成，为动态捕获的变长视频提供了一种有效的重拍生成解决方案，具有更好的泛化能力和视频质量。
...
TL;DR: 本文提出了Rectified SpaAttn方法，通过隐式全注意力参考来纠正注意力分配偏差，解决了现有稀疏注意力方法中关键令牌过度关注和非关键令牌完全忽略的问题，在HunyuanVideo和Wan 2.1上分别实现了3.33倍和2.08倍的加速，同时保持高质量生成。
...
Conclusion: Rectified SpaAttn通过纠正注意力分配偏差，有效解决了稀疏注意力方法的性能下降问题，在保持高质量生成的同时显著提升了计算效率。
...
TL;DR: 该论文提出了IMIC方法，通过在统一嵌入空间中同时训练四个任务（物体识别、高质量/低质量人脸识别、全身人物识别），有效解决了基础模型微调时的灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 解决视觉基础模型在微调时面临的灾难性遗忘问题，同时保持其在多个任务上的性能。
...
Conclusion: IMIC方法成功创建了在统一嵌入空间中处理多个任务的模型，实现了任务间的特征共享，同时避免了灾难性遗忘。
...
TL;DR: 提出STAvatar方法，通过UV自适应软绑定框架和时间自适应密度控制策略，从单目视频重建高保真可动画的3D头部化身，解决了现有方法在运动刚性和遮挡区域处理上的局限性。
...
Method: STAvatar包含两个关键组件：(1) UV自适应软绑定框架，利用图像和几何先验学习UV空间中每个高斯特征偏移；(2) 时间自适应密度控制策略，通过聚类结构相似帧并引入融合感知误差作为克隆准则。

Result: 在四个基准数据集上的广泛实验表明，STAvatar实现了最先进的重建性能，特别是在捕捉细粒度细节和重建频繁遮挡区域方面表现优异。

Conclusion: STAvatar通过创新的UV表示和时间密度控制策略，显著提升了3D头部化身的重建质量和动画能力，代码将公开可用。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将时间序列转换为伪图像进行时间预测，但未能建立语义级对齐；大型多模态模型在文本-图像对齐方面取得进展，但使用非视觉连续序列作为条件信号进行高保真图像生成的潜力尚未充分探索。
...
TL;DR: GigaWorld-0是一个统一的世界模型框架，通过视频生成和3D建模协同优化，为视觉-语言-动作学习提供高质量合成数据，使VLA模型在物理机器人上实现零真实交互训练下的强大性能。


<details>
  <summary>Details</summary>
Motivation: 世界模型正成为可扩展、数据高效具身AI的基础范式，需要开发能够生成视觉吸引人、空间一致、物理合理且指令对齐的具身交互数据的统一框架。
...
Conclusion: GigaWorld-0作为数据引擎，能够规模化合成具身交互数据，使VLA模型在零真实世界交互训练的情况下实现优异的物理机器人性能，为具身AI提供了有效的解决方案。
...
TL;DR: MAPS是一个用于Vision-Language-Action模型的鲁棒微调框架，通过模块级邻近调度来平衡稳定性和灵活性，无需额外参数或数据，在多个基准测试中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有的VLA模型微调方法要么过度约束适应能力，要么忽略了VLA组件不同角色的差异，导致破坏预训练表示并损害泛化能力。

Method: 提出MAPS框架，通过系统分析发现邻近约束松弛的实证顺序，并线性调度这种松弛，使视觉编码器保持接近预训练先验，而面向动作的语言层更自由地适应。

Result: 在MiniVLA-VQ、MiniVLA-OFT、OpenVLA-OFT等多个模型和基准测试中，MAPS一致提升了分布内和分布外性能（最高+30%），并在真实世界的Franka Emika Panda平台上验证有效。

Conclusion: 实证指导的与预训练VLM的邻近性是一个简单而强大的原则，可在VLM到VLA迁移中保持广泛的泛化能力。
...
TL;DR: ChessMamba是一个用于多时相遥感图像变化检测的结构感知框架，通过棋盘交错和蛇形扫描顺序解决时空错位问题，在多个变化检测任务中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 多时相遥感图像变化检测面临异质性和时空错位挑战，现有基于视觉Transformer或状态空间模型的方法在时间序列化过程中破坏了局部结构一致性，导致在错位情况下难以准确识别变化。

Method: ChessMamba集成了SpatialMamba编码器和轻量级跨源交互模块，采用棋盘交错与蛇形扫描顺序将多时相特征序列化为统一序列，并通过多空洞卷积进行结构感知融合，选择性捕获中心-角落邻域上下文。

Result: 在三个变化检测任务（二值变化检测、语义变化检测和多模态建筑损伤评估）上的综合评估表明，ChessMamba能有效融合异质特征，相比最先进方法实现了显著的精度提升。

Conclusion: ChessMamba通过结构感知的状态空间建模，在多时相遥感图像变化检测中实现了鲁棒的性能，特别是在处理时空错位和异质性方面表现出色。
...
TL;DR: 提出频率解耦的跨模态知识蒸馏方法，通过频域特征解耦和平衡不同模态间的知识迁移，在低频特征上强制对齐，高频特征上采用宽松对齐，显著提升跨模态知识蒸馏效果。


<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏在跨模态场景（如视觉到语言）中效果不佳，主要由于不同模态间表示不一致导致知识迁移困难。

Method: 基于频域分析，发现低频特征具有跨模态一致性，高频特征跨模态相似性低。采用不同损失函数：低频特征强对齐，高频特征宽松对齐，并引入尺度一致性损失和共享分类器统一特征空间。

Result: 在多个基准数据集上的实验表明，该方法显著优于传统知识蒸馏和最先进的跨模态知识蒸馏方法。

Conclusion: 频率解耦的跨模态知识蒸馏方法通过频域特征的有效解耦和平衡，成功解决了跨模态知识迁移的挑战，为模型压缩和性能提升提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有计算机辅助诊断(CAD)数据集通常只支持单一任务，限制了CAD技术的发展。为了探索任务间的相关性并避免训练过程中任务特定数据集间的异质性，需要构建一个多任务数据集。

Method: 基于动脉期增强CT图像，构建了包含150个病例的多任务肝脏数据集(LiMT)，涵盖四种肝脏疾病和正常病例，每个病例都经过经验丰富的临床医生仔细标注和校准。

Result: 创建了一个公开的多任务肝脏数据集，提供相关基线实验结果，并回顾了现有肝脏相关任务的数据库和方法。数据集包含肝脏和肿瘤分割、多标签病变分类和病变检测任务。

Conclusion: LiMT数据集可能成为医学影像研究社区未来的宝贵资源，为探索任务间相关性提供了解决方案，避免了训练过程中任务特定数据集间的异质性。
...
<details>
  <summary>Details</summary>
Motivation: 开源的大型视觉语言模型在科学视觉问答(SVQA)方面表现不佳，主要瓶颈在于缺乏大规模、高质量的公开数据集。现有使用LVLMs合成数据的方法存在系统性错误。

Method: 提出了生成-验证框架：首先生成带有图相关文本上下文的QA对，然后通过跨模态一致性检查和辅助过滤器来消除错误对，从而创建VeriSciQA数据集。

Result: 构建了包含20,351个QA对的VeriSciQA数据集，涵盖20个科学领域和12种图表类型。开源模型准确率仅64%，而专有模型达82%。在VeriSciQA上微调的模型在SVQA基准测试中表现持续提升。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统视觉语言智能体依赖人工标注监督的局限性，以及纯文本自评估在处理复杂视觉推理时的幻觉问题。

Method: 在单个LVLM中统一两个协同角色：执行多轮工具集成推理的Solver，以及通过工具基础批判生成结构化反馈和细粒度自奖励的Verifier，通过自进化推理循环实现稳定自我改进。

Result: 在几何问题求解和视觉科学分析任务上，相比基础模型实现了12.5%的性能提升。

Conclusion: Agent0-VL通过工具集成的自我评估和自我修复，能够实现无需外部奖励的持续自我改进，有效对齐推理和验证行为。
...
TL;DR: Motion Marionette是一个零样本框架，用于从单目源视频到单视图目标图像的刚体运动迁移。该方法通过内部先验而非外部先验来指导迁移过程，解决了通用性和时间一致性之间的权衡问题。
...
Method: 首先将源视频和目标图像提升到统一的3D表示空间，从源视频提取运动轨迹构建空间-时间先验，该先验独立于物体几何和语义。然后将该先验与目标对象集成以合成可控速度场，并使用基于位置的动力学进行细化。

Result: 实证结果表明，Motion Marionette能够泛化到不同对象，产生与源运动对齐的时间一致视频，并支持可控视频生成。

Conclusion: Motion Marionette通过内部空间-时间先验实现了有效的刚体运动迁移，在保持时间一致性的同时具有良好的通用性。
...
Method: 使用高斯采样从训练数据中初始化可学习动作查询，与推理增强的视觉语言特征交互，并行生成连续动作轨迹；整合8个自动驾驶数据集为标准化格式，结合监督学习和强化学习微调。
...
TL;DR: 该论文提出了一个框架，通过3D视觉数据预测复杂3D打印物体的光化学转化，引入了计算机视觉新任务：从3D视觉数据预测密集的非视觉体积物理特性。
...
Method: 提出了耦合物理门控适应（C-PGA）多模态融合架构，使用稀疏几何和工艺参数作为查询，通过特征级线性调制动态门控和适应密集视觉特征。

Result: 该方法在虚拟化学表征方面取得突破，无需传统打印后测量即可实现化学转化状态的精确控制。

Conclusion: C-PGA架构通过物理耦合建模，能够从3D视觉数据准确预测复杂物体的光化学转化状态。
...
TL;DR: 本文提出LoTTS框架，首次实现无需训练的局部测试时缩放方法，通过定位和修复图像中的缺陷区域来提升扩散模型质量，同时大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有测试时缩放方法在全图像层面操作，忽略了图像质量的局部差异性，导致对已满意区域进行不必要计算，同时无法充分修正局部缺陷。

Method: 提出LoTTS框架：1）通过对比交叉注意力和自注意力信号在质量感知提示下定位缺陷区域；2）仅对缺陷区域进行局部扰动和去噪，保持高质量区域不变。

Result: 在SD2.1、SDXL和FLUX模型上的实验表明，LoTTS在提升局部质量和全局保真度的同时，将GPU成本相比Best-of-N采样降低了2-4倍。
...
TL;DR: HybriDLA是一个新颖的文档布局分析框架，结合了扩散和自回归解码，解决了现代文档中元素数量多样和布局复杂的问题，在DocLayNet和M$^6$Doc基准测试中达到了83.5% mAP的最先进性能。


<details>
  <summary>Details</summary>
Motivation: 传统的文档布局分析方法依赖于经验先验或固定数量的可学习查询，在处理现代文档时面临挑战，因为现代文档具有多样化的元素数量和日益复杂的布局结构。

Method: HybriDLA在单一层中统一了扩散和自回归解码：扩散组件迭代优化边界框假设，自回归组件注入语义和上下文感知；还设计了多尺度特征融合编码器来捕获细粒度和高层次视觉线索。

Result: 在DocLayNet和M$^6$Doc基准测试中，HybriDLA达到了83.5%的平均精度(mAP)，超越了之前的方法，建立了最先进的性能。

Conclusion: HybriDLA通过结合扩散和自回归解码的统一框架，有效解决了现代文档布局分析的挑战，为处理复杂多样的文档布局提供了有效的解决方案。
...
TL;DR: DetVLM是一个新颖的智能图像检索框架，通过融合目标检测和视觉大模型的优势，实现了细粒度图像检索中的状态搜索和零样本搜索能力。
...
Method: 提出两阶段流水线：首先使用YOLO检测器进行高效、高召回率的组件级筛选；然后使用VLM作为召回增强单元，对检测器漏检的组件进行二次验证。

Result: 在车辆组件数据集上，DetVLM实现了94.82%的整体检索准确率，显著优于仅使用检测的基线方法。在零样本搜索中达到94.95%的准确率，在状态搜索任务中平均准确率超过90%。

Conclusion: DetVLM成功融合了目标检测和视觉大模型的优势，为细粒度图像检索提供了高效且强大的解决方案，特别是在状态搜索和零样本搜索方面表现出色。
...
### [55] [CounterVQA: Evaluating and Improving Counterfactual Reasoning in Vision-Language Models for Video Understanding](https://arxiv.org/abs/2511.19923)
*Yuefei Chen,Jiang Liu,Xiaodong Lin,Ruixiang Tang*

Main category: cs.CV

TL;DR: 本文提出了CounterVQA视频基准测试来评估视觉语言模型的反事实推理能力，发现现有模型在复杂因果链推理上表现不佳，并开发了CFGPT方法来提升这种能力。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在视频理解方面取得进展，但其反事实推理能力（推断假设条件下的替代结果）尚未充分探索，这对鲁棒的视频理解至关重要。

Method: 引入CounterVQA基准测试评估反事实推理能力，开发CFGPT后训练方法，通过从语言模态蒸馏反事实推理能力来增强模型的视觉反事实推理。

Result: 评估显示现有模型在简单反事实问题上表现尚可，但在复杂多跳因果链上性能显著下降；CFGPT方法在所有难度级别上均带来一致改进。

Conclusion: 反事实推理是视觉语言模型的重要能力，现有模型在这方面存在局限，CFGPT方法能有效提升模型的视觉反事实推理能力。
...
TL;DR: CPDATrack是一个基于Transformer的跟踪框架，通过概率驱动的注意力机制来抑制背景和干扰物令牌的干扰，同时保持目标周围的上下文信息，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有的一流Transformer跟踪器通过连接模板和搜索区域令牌实现联合注意力，但过多的背景搜索令牌关注目标模板会削弱跟踪器的判别能力。现有的令牌剪枝方法往往会删除目标附近的令牌，导致重要上下文信息丢失。

Method: 提出CPDATrack框架：1）在指定编码器层之间集成可学习模块，估计每个搜索令牌与目标关联的概率；2）基于概率估计剪除信息量较少的背景令牌，同时保留目标周围的上下文线索；3）使用判别性选择性注意力机制，在早期层完全阻止搜索到模板的注意力；4）在后续编码器层中，从局部区域选择性提取高概率目标令牌来关注模板令牌。
...
TL;DR: 本文提出DRIFT框架，利用预训练图像扩散模型的自注意力机制作为语义标签传播核，实现零样本视频目标跟踪和分割，无需额外训练即可在标准基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 图像扩散模型在生成过程中隐式捕获了丰富的语义结构，这些结构可以用于识别和定位任务。本文旨在探索如何利用扩散模型的自注意力图作为语义标签传播核，实现零样本的视频目标跟踪。

Method: 将扩散模型的自注意力图重新解释为语义标签传播核，构建时间传播核实现帧间标签传播。采用测试时优化策略（DDIM反转、文本反转和自适应头权重）来增强扩散特征的鲁棒性。结合SAM引导的掩码细化，提出DRIFT框架。

Result: 在标准视频目标分割基准测试中，DRIFT框架实现了最先进的零样本性能，证明了扩散模型自注意力机制在语义标签传播方面的有效性。

Conclusion: 扩散模型的自注意力机制可以作为强大的语义标签传播工具，通过适当的优化策略和框架设计，能够在零样本设置下实现高质量的视频目标跟踪和分割。
...
TL;DR: 本文提出了高分辨率图像编辑任务，并开发了一个测试时优化框架来解决该问题，通过分块优化、细节传递和同步策略实现高质量的高分辨率图像编辑。
...
Method: 采用测试时优化框架，对高分辨率源图像进行分块优化，然后通过细粒度细节传递模块和新型同步策略来保持分块间的一致性。

Result: 大量实验表明，该方法能够产生高质量的编辑结果，为高分辨率内容创建铺平了道路。

Conclusion: 提出的方法成功解决了高分辨率图像编辑的挑战，通过分块优化和同步策略实现了高质量的结果，推动了高分辨率内容创建的发展。
...
TL;DR: SPROUT是一个无需训练和标注的核实例分割框架，利用组织学先验构建特定切片的参考原型，通过部分最优传输方案进行特征对齐，生成正负点提示引导Segment Anything Model实现精确的核分割。


<details>
  <summary>Details</summary>
Motivation: 现有方法大多依赖密集监督和计算昂贵的微调，而无需训练的方法在生物医学分割领域仍未被充分探索，需要开发可扩展的训练免费核实例分割新范式。

Method: 利用组织学先验构建特定切片的参考原型来缓解领域差距，通过部分最优传输方案逐步引导特征对齐，将前景和背景特征转换为正负点提示，无需参数更新即可使用SAM模型进行核分割。

Result: 在多个组织病理学基准测试上的广泛实验表明，SPROUT在无监督和重新训练的情况下实现了具有竞争力的性能。
...
TL;DR: 本文提出HiCoGen框架，通过链式合成范式和强化学习解决复杂提示下扩散模型的概念遗漏和构图问题。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在处理涉及多个对象和层次结构的复杂提示时，难以准确遵循指令，导致概念遗漏、混淆和构图能力差的问题。

Method: 提出分层组合生成框架HiCoGen，基于链式合成范式：首先使用大语言模型将复杂提示分解为最小语义单元，然后迭代合成这些单元，每个步骤生成的图像为下一步提供视觉上下文。引入强化学习框架，提出衰减随机性调度来增强探索，并采用分层奖励机制。
...
<details>
  <summary>Details</summary>
Motivation: 现有4D方法依赖外部先验、繁重后优化或需要4D数据集微调，而VGGT等3D基础模型在动态对象主导时性能显著下降，需要一种无需训练就能扩展3D基础模型进行稳健4D重建的方法。

Method: 通过gram相似性挖掘并放大VGGT全局注意力层中的动态线索，跨时间窗口聚合，引入投影梯度驱动的细化策略锐化掩码边界，并将精确掩码集成到VGGT早期推理中。

Result: 在六个数据集上，该方法在动态对象分割、相机姿态估计和密集重建方面实现卓越性能，支持单次推理超过500帧的长序列。

Conclusion: VGGT4D证明了VGGT全局注意力层已隐含编码丰富的动态线索，通过挖掘这些线索可以无需训练地实现稳健的4D场景重建。
...
Conclusion: EmoFeedback2方法通过LVLM的推理能力提供奖励和文本反馈，显著提升了连续情感图像生成的情感连续性和情感保真度。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于引导的方法在理论上可用于图像修复等逆问题，但实际效果有限，导致需要专门的修复模型。本文旨在开发无需训练的通用修复方法。

Method: 优化初始种子噪声以匹配未遮罩区域，采用线性近似避免计算展开，在频域进行优化以提高稳定性。
...
TL;DR: 提出了一种结合时间注视模式、头部运动数据和视觉场景信息的多模态VR注视预测方法，通过门控融合机制和跨模态注意力自适应加权不同模态信息，在22个VR场景的530万注视样本数据集上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 虚拟现实环境中的注视行为预测对于渲染优化和界面设计具有重要意义，但目前仍面临挑战。本文旨在通过多模态方法提高VR注视预测的准确性。

Method: 采用多模态方法结合时间注视模式、头部运动数据和视觉场景信息，使用门控融合机制和跨模态注意力来自适应加权不同模态的上下文相关性。

Result: 在22个VR场景的530万注视样本数据集上评估，多模态组合相比单独使用单一数据流提高了预测准确性，在1-3个未来帧的预测中表现更好，跨场景泛化测试显示93.1%的验证准确率和时间一致性。

Conclusion: 该方法有助于理解虚拟环境中的注意力机制，在渲染优化、交互设计和用户体验评估方面具有应用潜力，为实现无需昂贵眼动追踪硬件就能预测用户注意力模式的更高效虚拟现实系统迈出了一步。
...
TL;DR: 本文提出了一种细节感知的细化框架，通过两阶段参考驱动校正来增强像素级一致性，解决了当前扩散模型在参考图像引导下难以保留细粒度视觉细节的问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于VAE的潜在压缩方法会丢失细微纹理信息，导致身份和属性特定线索消失。现有的后编辑方法在放大局部细节时往往与原始图像在光照、纹理或形状上不一致。

Method: 采用两阶段参考驱动校正：首先通过微调单图像扩散编辑器，使其同时接收草稿图像和参考图像，实现全局一致的细化；然后应用强化学习进一步强化局部编辑能力，明确优化细节准确性和语义一致性。

Result: 广泛的实验表明，该方法显著提高了参考对齐和细粒度细节保留能力，在具有挑战性的参考引导恢复基准测试中超越了开源和商业模型。
...
TL;DR: 本文提出了第一个类型特定的创造力奖励模型CREward，涵盖几何、材质和纹理三个创造力维度，通过人类基准评估和大型视觉语言模型生成标签来训练模型，并应用于创造力评估、可解释创造力和创意样本获取。


<details>
  <summary>Details</summary>
Motivation: 创造力是一个复杂现象，将其视为单一未分化的数量显得过于简单。本文旨在通过图像形成流程的视角，学习类型特定的创造力奖励模型，以更精细地表示和评估创造力。

Method: 首先进行人类基准评估以捕捉人类对各类创意图像的创造力感知，分析人类判断与大型视觉语言模型预测的相关性，然后收集LVLM生成的标签来训练CREward模型。

Result: 确认大型视觉语言模型与人类感知具有强对齐性，成功训练出适用于创意图像评估和生成的CREward模型，并探索了三个应用方向。

Conclusion: CREward模型能够有效评估和生成创意图像，为创造力研究提供了新的工具和方法，在创造力评估、解释和样本获取方面具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界模型通常进行一系列决策，孤立错误容易被纠正，但级联错误可能导致严重风险。传统对抗攻击只关注单次决策操纵，缺乏对决策链劫持的研究。

Method: 提出语义感知通用扰动(SAUPs)，在归一化空间中通过语义分离策略搜索扰动，开发有效算法克服优化挑战。

Result: 在三个多模态大语言模型上的广泛实验显示其脆弱性，使用单个对抗帧控制五个不同目标时达到70%的攻击成功率。

Conclusion: SAUPs暴露了模型在决策链劫持方面的严重安全威胁，需要开发更鲁棒的防御机制来应对这种级联错误风险。
...
TL;DR: 提出了一种注意力引导的跨模态交互Transformer（ACIT）用于预测行人过街意图，通过六种视觉和运动模态的三组交互对，结合双路径注意力机制和多模态特征融合，在JAAD数据集上取得了优于现有方法的表现。
...
Method: ACIT利用六种视觉和运动模态，分为三组交互对：全局语义图和全局光流、局部RGB图像和局部光流、自车速度和行人边界框。采用双路径注意力机制增强主模态的显著区域，并通过光流引导注意力与辅助模态深度交互。使用跨模态注意力建模跨模态动态，并引入Transformer时序特征聚合模块捕捉序列依赖。

Result: 在JAADbeh和JAADall数据集上分别达到70%和89%的准确率，优于现有最先进方法。消融研究验证了各模块的贡献。

Conclusion: ACIT通过有效的跨模态交互和时序建模，在行人过街意图预测任务中表现出色，为自动驾驶安全提供了有力支持。
...
<details>
  <summary>Details</summary>
Motivation: MLLMs在视觉注意力方面定位能力有限，导致产生幻觉。虽然能准确提取视觉语义，但在后续推理中未能充分利用这一优势。

Method: 提出VGA训练免费方法：1）利用视觉token语义构建精确视觉基础；2）用此基础引导模型关注相关视觉区域；3）在图像描述任务中动态抑制已描述区域。

Result: VGA仅引入4.36%的延迟开销，与FlashAttention等高效注意力实现完全兼容。在多个MLLMs和幻觉基准测试中达到最先进的去幻觉性能。

Conclusion: 明确的视觉引导在增强MLLMs视觉理解能力中起关键作用，VGA有效提升了模型的视觉定位和推理能力。
...
TL;DR: COVec是一个基于明暗对比原理的感知光照向量化框架，首次在向量域引入内在图像分解，将图像分离为反照率、阴影和光照层，通过语义引导初始化和两阶段优化实现高保真度和可编辑性的向量表示。


<details>
  <summary>Details</summary>
Motivation: 现有向量化方法在处理复杂真实世界图像时存在困难，通常会产生碎片化形状，牺牲了语义简洁性。

Method: 提出COVec框架，引入内在图像分解到向量域，将图像分离为反照率、阴影和光照层；采用语义引导初始化和两阶段优化，结合可微分渲染进行细化。

Result: 在多个数据集上的实验表明，COVec相比现有方法实现了更高的视觉保真度和显著改善的可编辑性。

Conclusion: COVec通过光照感知的向量化方法，成功解决了复杂图像向量化中的保真度和可编辑性问题，为图像向量化提供了新的解决方案。
...
Conclusion: MFM-Point框架显著提升了基于点方法的可扩展性和性能，证明了多尺度方法在点云生成中的有效性，同时保持了方法的简单性和效率。
...
TL;DR: HACBSR是一个无监督的盲超分辨率框架，无需真实图像和外部核先验，通过对比核采样和历史增强对比学习来解决行星遥感图像的质量问题。


<details>
  <summary>Details</summary>
Motivation: 行星遥感图像受成像环境和硬件限制影响，缺乏真实图像数据，限制了监督盲超分辨率方法的性能。

Method: 包含对比核采样机制（缓解高斯采样分布偏差）和历史增强对比学习（使用历史模型生成负样本，实现非贪婪优化和强凸性）。

Result: 在多个放大因子下，HACBSR与最先进的无监督方法相比具有竞争力，并引入了Ceres-50数据集用于评估。

Conclusion: HACBSR为无监督盲超分辨率提供了一种有效解决方案，特别适用于缺乏真实图像的行星遥感应用。
...
TL;DR: 本文提出了一种新的任务-步骤-状态（TSS）框架，通过引入状态作为视觉基础语义层，将抽象程序与可观察的视觉细节联系起来，从而改进过程感知视频表示学习。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过任务和步骤级别的文本-视觉对齐来注入过程语义，但由于任务和步骤描述的高度抽象性，难以与视觉数据中的具体细节形成稳健对齐。
...
Method: 利用CEST数据的自相似性推导自适应方差稳定变换，在数据线性变换上进行两阶段去噪，使用局部SVD分解作为线性变换以防止空间和谱去噪伪影。

Result: 在多个体模和体内CEST扫描实验中，BALD在去噪指标和下游任务（如分子浓度图估计和癌症检测）方面均优于现有最先进的CEST去噪方法。

Conclusion: BALD方法有效解决了CEST MRI中的噪声问题，提高了定量对比映射的准确性，在生物医学应用中表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉异常检测模型虽然能提供视觉解释，但缺乏直接且语义上有意义的解释，限制了用户对异常的理解。
...
Conclusion: 概念感知的视觉异常检测方法成功地将语义解释与定位解释相结合，为VAD系统提供了更深入的可解释性。
...
Method: 开发可训练奖励模型，通过将候选轨迹与世界模型预测的未来动态对齐，将世界知识注入教师策略；然后通过策略蒸馏和世界奖励蒸馏将教师推理能力转移到学生策略。

Result: 在开环和闭环基准测试中，WPT达到最先进性能：0.11碰撞率（开环）和79.23驾驶分数（闭环），超越基于世界模型和模仿学习方法；学生策略推理速度提升4.9倍。
...
TL;DR: 本文提出了一个用于早期森林火灾检测的数据集，通过游戏模拟器合成烟雾和初始火灾图像，结合现有数据集，并比较了YOLOv7和检测变换器模型在火灾检测任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的火灾检测方法由于缺乏大规模数据集和专门针对早期火灾检测的模型，存在漏检问题。需要开发能够识别初始烟雾和火苗的早期预警系统。

Method: 利用Red Dead Redemption 2等游戏模拟器合成包含烟雾羽流和初始火灾的图像数据集，结合已发布的图像构建综合数据集，并比较YOLOv7和检测变换器模型在图像分类和定位任务上的表现。

Result: 创建了一个专门针对早期森林火灾检测的数据集，包含多个烟雾羽流和初始火灾实例，为早期火灾预警提供了数据支持。

Conclusion: 通过游戏模拟器合成的数据集能够有效支持早期森林火灾检测模型的开发，YOLOv7和检测变换器模型在该任务上表现出良好的检测性能。
...
TL;DR: 本文提出了一种结合深度学习工具和注意力机制的集成方法，用于通过X射线图像自动检测心脏肥大。该方法使用Inception V3模型和多层注意力机制，在评估阶段取得了优异性能。
...
Method: 采用数据收集和预处理阶段优化图像质量，使用CNN配置的Inception V3模型作为核心模块，并集成多层注意力机制，特别是多头注意力机制来自动学习特征并选择性关注输入图像的特定区域。

Result: 模型在评估中表现出色：准确率95.6%，精确率95.2%，召回率96.2%，灵敏度95.7%，特异性96.1%，AUC为96.0。

Conclusion: 该方法能够敏感地识别心脏肥大，证明了其在临床诊断中的实用价值和有效性。
...
TL;DR: LungEvaty是一个基于transformer的框架，用于从单次低剂量CT扫描预测1-6年肺癌风险。该模型处理全肺输入，直接从大规模筛查数据中学习，无需像素级注释或区域监督，性能达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 随着更多国家引入基于低剂量CT的群体筛查项目，肺癌风险评估变得越来越重要。现有方法要么过度依赖像素级注释限制了可扩展性，要么将肺部分析为片段削弱了性能。

Method: 提出完全基于transformer的LungEvaty框架，处理全肺输入，可选使用解剖学感知注意力引导损失来鼓励解剖学聚焦的注意力。模型仅使用影像数据，无需区域监督。

Result: 在超过90,000个CT扫描上训练，包括28,000多个用于微调和6,000个用于评估。LungEvaty匹配最先进性能，提供简单、数据高效且完全开源的解决方案。

Conclusion: 该框架为未来纵向和多模态肺癌风险预测研究提供了可扩展的基础，能够有效处理大规模筛查数据集。
...
TL;DR: 本文提出UltraViCo方法，通过抑制训练窗口外token的注意力来解决视频扩散变换器的长度外推问题，将外推限制从2倍提升到4倍。


<details>
  <summary>Details</summary>
Motivation: 视频扩散变换器在超出训练长度时会出现周期性内容重复和普遍质量下降的问题，现有方法仅通过位置编码解决重复问题，忽略了质量下降且外推能力有限。

Method: 从注意力图角度分析问题根源，发现注意力分散是导致两种失败模式的统一原因，提出训练即插即用的UltraViCo方法，通过恒定衰减因子抑制训练窗口外token的注意力。

Result: 在多种模型和外推比例下显著优于基线方法，在4倍外推时动态度和图像质量分别比之前最佳方法提升233%和40.5%，并能无缝推广到可控视频合成和编辑等下游任务。

Conclusion: 注意力分散是视频长度外推问题的根本原因，UltraViCo通过联合解决内容重复和质量下降两个问题，成功将外推限制从2倍扩展到4倍。
...
TL;DR: 提出了PETRG-3D框架，用于自动化PET/CT报告生成，解决了功能成像的独特挑战，并在多中心淋巴瘤数据集上验证了其优越性能。


<details>
  <summary>Details</summary>
Motivation: PET/CT在肿瘤学中至关重要，但扫描仪快速扩张超过了专业人员的可用性，需要自动化报告生成来减轻临床工作负担。功能PET成像面临代谢模式随示踪剂生理变化和需要全身3D上下文信息等独特挑战。

Method: 提出PETRG-3D端到端3D双分支框架，分别编码PET和CT体积，并加入风格自适应提示以减轻医院间报告实践的差异。构建了多中心淋巴瘤数据集PETRG-Lym和公开基准AutoPET-RG-Lym。

Result: PETRG-3D在自然语言指标（如ROUGE-L提升31.49%）和临床效能指标（如PET-All提升8.18%）上显著优于现有方法，证明了体积双模态建模和风格感知提示的益处。
...
TL;DR: Restora-Flow是一种无需训练的流匹配方法，通过退化掩码引导采样并加入轨迹校正机制，在图像修复任务中实现更快的处理速度和更好的感知质量。
...
Result: 在自然和医学数据集上的多种图像修复任务（修复、超分辨率、去噪）中，相比扩散模型和流匹配基准方法，展现出更优的感知质量和更快的处理时间。

Conclusion: Restora-Flow通过结合掩码引导和轨迹校正，为基于流匹配的图像修复提供了一种高效且高质量的解决方案。
...
TL;DR: 提出了R-TNAG框架，通过黎曼流形映射、时间感知神经常微分方程和注意力机制门控循环单元，解决不规则采样纵向sMRI数据中阿尔茨海默病进展建模的挑战。


<details>
  <summary>Details</summary>
Motivation: 临床检查的不确定性导致纵向成像数据观测间隔不规则，现有基于欧几里得空间的模型无法充分捕捉不规则采样纵向图像的内在连续性和非线性几何结构。
...
TL;DR: MAP-World是一个无先验的多模态规划框架，结合掩码动作规划和路径加权世界模型，用于自动驾驶运动规划，无需手工锚点或强化学习即可生成多样化轨迹。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶规划系统依赖手工锚点或强化学习来选择单一最佳轨迹模式，这丢弃了替代未来信息并使优化复杂化。

Method: 使用掩码动作规划将未来自车运动视为序列补全，通过注入噪声生成多样化轨迹查询；轻量级世界模型基于候选轨迹展开未来BEV语义；训练时使用轨迹概率作为离散路径权重计算语义损失期望。
...
<details>
  <summary>Details</summary>
Motivation: 现有持续视觉指令调优研究主要关注未安全对齐的模型，忽视了真实世界多模态大语言模型需要安全机制来降低潜在风险。本文发现安全对齐模型在持续适应过程中不仅会出现任务遗忘，还会出现安全性退化。

Method: 提出和谐参数适应（HPA）框架，包括基于关注度的参数划分、和谐平衡的参数选择和正交参数调整。将参数按关注安全或任务性能进行划分，从平衡角度选择需要保留的参数，并施加正交性约束来缓解灾难性遗忘。

Result: 在持续视觉指令调优基准和安全评估数据集上的广泛实验表明，HPA相比现有基线方法能更好地保持高安全性和减轻遗忘。

Conclusion: HPA框架有效解决了安全对齐多模态大语言模型在持续适应过程中的安全性与任务性能平衡问题，为实际应用提供了重要解决方案。
...
Method: 构建ADNet基准，从49个公开数据集中聚合380个类别，包含196,294张标准化RGB图像，提供像素级注释和结构化文本描述。提出Dinomaly-m方法，基于上下文引导的专家混合扩展，在不增加推理成本的情况下扩展解码器容量。

Result: 实验显示现有SOTA方法在一对一设置下达到90.6% I-AUROC，但在扩展到380个类别的多类设置中降至78.5%。Dinomaly-m方法达到83.2% I-AUROC和93.1% P-AUROC，优于现有方法。

Conclusion: ADNet作为标准化可扩展基准，支持社区扩展异常检测数据集，为未来异常检测基础模型提供可扩展基础。大规模基准揭示了现有方法的可扩展性挑战，而Dinomaly-m方法有效解决了这一问题。
...
TL;DR: Exo2EgoSyn是基于WAN 2.2的跨视角视频生成框架，实现了从第三人称视角到第一人称视角的视频合成，无需从头训练。
...
<details>
  <summary>Details</summary>
Motivation: 视频文本视觉问答任务面临重大挑战，需要模型准确感知和理解视频中不同尺度、方向和清晰度的场景文本，同时有效整合时间和语义上下文来生成精确答案，并过滤冗余信息。

Method: 提出SFA训练免费框架，模拟人类答题过程，通过自适应扫描视频帧、选择性关注关键区域并直接放大这些区域，引导视频大语言模型关注关键线索。

Result: SFA在多个公开视频文本视觉问答数据集上取得了新的最先进结果，相比之前方法有显著提升，证明了其有效性和泛化能力。

Conclusion: SFA框架通过引导视频大语言模型关注关键线索，有效解决了视频文本视觉问答任务中的挑战，在多个数据集上表现出色。
...
TL;DR: 本文介绍了参加2025年ASNR-MICCAI BraTS-Inpainting挑战赛的获胜方案，使用U-Net架构结合随机掩码增强策略，在脑肿瘤MRI图像中合成健康组织，取得了最佳性能。


<details>
  <summary>Details</summary>
Motivation: 解决脑肿瘤MRI定量分析中深度学习模型受数据集偏差限制的问题，通过合成健康组织来减少数据偏差。
...
Conclusion: 该方法在BraTS-Inpainting 2025挑战赛中荣获第一名，性能超越了2023和2024年的获胜方案。
...
TL;DR: OmniAlpha是首个统一的序列到序列RGBA图像生成和编辑多任务生成框架，通过新颖的MSRoPE-BiL方法和AlphaLayers数据集，在21个多样化任务上超越专用模型。


<details>
  <summary>Details</summary>
Motivation: 生成模型在RGB合成方面表现出色，但实际应用需要RGBA操作。现有模型要么是专门处理alpha但缺乏多功能性，要么是统一多任务框架但局限于RGB领域。

Method: 提出OmniAlpha框架，采用MSRoPE-BiL（一种新颖的RoPE方法，具有双向可扩展层轴）作为其Diffusion Transformer骨干，支持同时处理多个输入和目标RGBA层。构建AlphaLayers数据集，包含1000个高质量多层三元组。

Result: 在21个多样化任务上联合训练，统一方法持续优于强专用基线。在AIM-500上实现掩码自由抠图的SAD相对减少84.8%，在层条件完成中获得超过90%的人类偏好。

Conclusion: 证明统一多任务模型可以学习RGBA的优越共享表示，为更强大的层感知生成系统铺平道路。
...
TL;DR: 本文提出CT-CIG方法，通过可控文本引导生成逻辑合理的伪装图像，利用大视觉语言模型标注高质量文本提示，并设计频率交互细化模块提升伪装纹理效果。


<details>
  <summary>Details</summary>
Motivation: 现有伪装图像生成方法往往忽略伪装对象与背景环境的逻辑关系，导致生成结果不自然。本文旨在解决这一问题，生成既真实又逻辑合理的伪装图像。

Method: 提出CT-CIG方法：1）设计伪装揭示对话机制，利用大视觉语言模型标注高质量文本提示；2）微调Stable Diffusion，加入轻量控制器指导伪装对象位置和形状；3）设计频率交互细化模块捕获高频纹理特征。

Result: 通过CLIPScore评估和伪装效果测试，证明生成的文本提示具有语义对齐性，CT-CIG能够生成逼真的伪装图像。

Conclusion: CT-CIG方法能够有效生成逻辑合理且逼真的伪装图像，在语义对齐和视觉效果方面均优于现有方法。
...
TL;DR: 该研究针对胶质母细胞瘤的异质性问题，开发了一种基于预训练Vision Transformer的深度学习模型，在BraTS-Path 2025挑战赛中获得了第二名。
...
Conclusion: 该研究为基于ViT的组织病理学分析建立了坚实的基线，未来工作将专注于缩小在未见验证数据上的性能差距。
...
TL;DR: V-Attack是一种针对大型视觉语言模型的精确局部语义攻击方法，通过操作注意力机制中的值特征而非传统补丁特征，解决了现有方法在语义控制精度上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击方法在大型视觉语言模型上难以精确控制特定概念的语义，主要原因是补丁-令牌表示中的语义纠缠问题。

Method: 提出V-Attack方法，包含两个核心组件：自值增强模块和文本引导值操作模块，通过直接操作注意力机制中的值特征来实现精确语义控制。

Result: 在LLaVA、InternVL、DeepseekVL和GPT-4o等多种LVLM上的实验表明，V-Attack相比现有最先进方法的攻击成功率平均提高了36%。

Conclusion: V-Attack揭示了现代视觉语言理解系统中的关键漏洞，证明了值特征作为精确语义操作手柄的有效性。
...
TL;DR: 提出HistoSpeckle-Net深度学习架构，用于从多模光纤散斑重建结构丰富的医学图像，通过分布感知学习策略和特征细化模块，在有限数据和光纤弯曲条件下实现高质量重建。


<details>
  <summary>Details</summary>
Motivation: 现有多模光纤成像方法主要针对简单数据集，难以处理复杂真实世界成像任务，且通常需要大量数据。本文旨在开发能够处理复杂医学图像、减少数据依赖的实用多模光纤成像方法。

Method: 开发光学实验装置，使用空间光调制器将激光耦合到多模光纤中，捕获OrganAMNIST图像的输出散斑模式。提出HistoSpeckle-Net架构，包含基于直方图的互信息损失和三尺度特征细化模块，结合多尺度结构相似性损失。

Result: 在复杂OrganAMNIST数据集上的实验表明，HistoSpeckle-Net比U-Net和Pix2Pix等基线模型具有更高的保真度，在有限训练样本和不同光纤弯曲条件下均表现出优越性能。

Conclusion: HistoSpeckle-Net通过有效重建复杂解剖特征，在减少数据需求和光纤扰动条件下，使多模光纤成像更接近实际临床环境部署。
...
TL;DR: 本文研究了文本到图像生成模型在长提示词下的表现，发现长提示词虽然提高了保真度但抑制了多样性。作者提出了LPD-Bench基准和PromptMoG方法，通过混合高斯采样来增强多样性。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成模型在长提示词下表现出保真度与多样性的权衡问题，长提示词编码丰富信息但往往导致输出重复且缺乏创造性。

Method: 作者提出了LPD-Bench评估基准，并开发了PromptMoG方法，该方法在嵌入空间中使用混合高斯分布采样提示词嵌入，无需训练即可增强多样性。

Result: 在SD3.5-Large、Flux.1-Krea-Dev、CogView4和Qwen-Image四个先进模型上的实验表明，PromptMoG能一致地提高长提示词生成的多样性，同时保持语义一致性。

Conclusion: PromptMoG是一种有效的训练无关方法，能够解决长提示词生成中的多样性问题，为文本到图像生成提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界环境需要能够识别多样化、未见过的物体的模型，这是闭集方法的主要限制。现有开放词汇3D检测器虽然放宽了标注要求，但仍依赖训练场景。
...
<details>
  <summary>Details</summary>
Motivation: 当前强化学习图像生成方法依赖预训练偏好模型输出标量奖励，但这些奖励往往无法准确捕捉人类感知，容易受到奖励黑客攻击，即高分不一定对应更好的图像质量。

Method: 提出Adv-GRPO框架，使用对抗性奖励迭代更新奖励模型和生成器。奖励模型以参考图像作为正样本进行监督学习，避免被黑客攻击。采用图像本身作为奖励，结合参考图像和视觉基础模型（如DINO）提供密集的视觉奖励信号。

Result: 在人类评估中，Adv-GRPO在图像质量和美学方面分别以70.0%和72.4%的胜率优于Flow-GRPO和SD3。该方法在图像质量、美学和任务特定指标上均取得一致提升。

Conclusion: 结合参考样本和基础模型奖励的方法能够实现分布转移和灵活的样式定制，有效解决了传统奖励函数的固有偏见问题，生成了更高质量的图像。
...
<details>
  <summary>Details</summary>
Motivation: 传统权重平均方法在多模态领域泛化中面临挑战：不同模态优化速度差异导致早期偏向快速收敛模态，抑制了较慢但互补模态的贡献，阻碍有效模态融合并导致损失曲面偏向更尖锐、泛化性较差的最小值。

Method: MBCD框架包含三个关键组件：1）学生模型中的自适应模态丢弃，抑制早期对主导模态的偏向；2）梯度一致性约束，对齐单模态分支与融合表示的学习信号；3）基于权重平均的教师模型进行跨模态蒸馏，将融合知识传输到每个单模态分支。

Result: 在MMDG基准测试上的广泛实验表明，MBCD始终优于现有方法，在不同未见领域上实现了更高的准确性和鲁棒性。

Conclusion: MBCD框架成功保留了权重平均的平坦化优势，同时克服了其在多模态上下文中的缺点，通过协调优化和跨模态知识转移，引导收敛到更平坦的解。
...
TL;DR: DiDiCM是一种新颖的扩散分类建模框架，利用扩散过程建模类别标签的后验分布，在不确定性条件下优于传统分类器。


<details>
  <summary>Details</summary>
Motivation: 传统分类方法在图像损坏或训练数据有限等高不确定性条件下表现不佳，需要更好的分类建模方法。
...
Conclusion: DiDiCM框架在不确定性条件下显著提升了分类性能，为图像分类任务提供了新的有效方法。
...
Method: 结合神经批处理采样器、自动编码器和预测器的强化学习框架，采样器通过复合奖励自适应选择信息丰富的图像块，自动编码器生成损失分布图突出异常区域，预测器在损失分布空间进行分割。

Result: 在MVTec AD数据集上实验表明，该方法比现有最优方法具有更高准确率和更好的细微异常定位能力，F1_max平均提升0.15，AUC平均提升0.06，最佳情况下F1_max最大提升0.37。
...
TL;DR: ScenarioCLIP模型通过显式建模对象间关系和组合结构，改进了传统CLIP模型在场景分析中的局限性，在跨模态检索和细粒度视觉理解任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统CLIP模型主要关注单对象分类或短描述检索，无法有效处理现实场景中丰富的组合结构。现有改进方法仍局限于预定义类别列表，缺乏对关系结构的显式建模。

Method: 提出ScenarioCLIP模型，接受输入文本、基础关系、输入图像以及突出关系的聚焦区域。通过预训练策划的场景数据，并在专门的下游任务上进行微调。构建了新的数据集，使用语言模型管道来基础化动作、对象和关系。

Result: ScenarioCLIP在各种领域特定任务中展示了强大的零样本和微调性能，在多个场景基准任务上优于基线方法。

Conclusion: ScenarioCLIP通过显式建模场景中的组合关系，有效提升了跨模态检索和细粒度视觉理解能力，为场景分析提供了新的解决方案。
...
TL;DR: SelfMOTR是一个新颖的跟踪转换器，利用自生成的检测先验来解决端到端跟踪中检测性能差和检测与关联冲突的问题，在DanceTrack数据集上达到与最新端到端跟踪方法竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管基于转换器的端到端跟踪架构取得进展，但检测性能差以及检测与关联在联合架构中的冲突仍然是关键问题。现有方法通过使用高级去噪或标签分配策略，或通过蒸馏或锚点提案技术从外部对象检测器引入检测先验来缓解这些问题。

Method: 受集成检测先验成功和MOTR类模型实际上是强大检测模型的启发，提出了SelfMOTR，这是一种依赖自生成检测先验的新型跟踪转换器。通过广泛分析和消融研究，揭示并展示了MOTR类模型的隐藏检测能力，并提供了有效利用这些能力的实用工具集。

Result: 在DanceTrack数据集上，SelfMOTR实现了强大的性能，与最新的端到端跟踪方法竞争。

Conclusion: SelfMOTR通过利用自生成的检测先验，有效解决了端到端跟踪中的检测性能问题，证明了MOTR类模型在检测方面的潜在能力，为多目标跟踪领域提供了新的解决方案。
...
TL;DR: 提出了一种基于大语言模型和视觉语言模型的迭代自优化框架，通过多模态思维链过程改进提示词，解决视频生成中的物理一致性不足问题。
...
Result: 在PhyIQ基准测试中，该方法将物理智商分数从56.31提升至62.38，显著改善了物理一致性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的参数高效微调方法在处理大规模地球观测任务时往往失效，因为它们无法充分处理遥感数据中固有的多面性和不可预测的域差距（如空间、语义和频率偏移）。

Method: 1. 建立全面的遥感模块工具箱，包含空间、语义和频率模块来处理多面域差距；2. 开发基于Fisher信息的自适应选择机制，量化每个模块的重要性并动态激活关键模块。

Result: 在16个跨域遥感语义分割基准测试中，CrossEarth-Gate方法达到了最先进的性能，验证了方法的有效性和泛化能力。

Conclusion: CrossEarth-Gate通过模块化工具箱和自适应选择机制，成功解决了遥感数据中的多面域差距问题，为遥感基础模型的高效微调提供了有效解决方案。
...
TL;DR: TReFT是一种新方法，通过直接使用预训练RF模型预测的速度作为输出来实现一步图像翻译，解决了对抗训练中的收敛问题，使RF模型能够实时进行图像翻译。
...
<details>
  <summary>Details</summary>
Motivation: 红外小目标检测面临信噪比低、背景复杂、目标特征不明显等挑战，现有深度学习方法在多样化场景下存在模式漂移问题，限制了鲁棒性。
...
TL;DR: AMB3R是一个多视图前馈模型，用于度量尺度的密集3D重建，能够处理多种3D视觉任务。该方法使用稀疏但紧凑的体素场景表示作为后端，仅通过多视图重建训练即可扩展到未标定视觉里程计和大规模运动结构估计，无需任务特定微调或测试时优化。


<details>
  <summary>Details</summary>
Motivation: 解决现有3D重建方法在任务扩展性、计算效率和性能方面的限制，开发一个统一的模型来处理多种3D视觉任务，同时保持空间紧凑性和几何推理能力。

Method: 使用稀疏紧凑的体素场景表示作为后端表示，构建多视图前馈模型。模型仅通过多视图重建任务进行训练，但可以无缝扩展到视觉里程计和运动结构估计等任务。

Result: 在相机姿态、深度和度量尺度估计、3D重建等方面达到最先进性能，甚至超过了基于优化的SLAM和SfM方法，在常见基准测试中表现出色。

Conclusion: AMB3R证明了单一模型可以有效地处理多种3D视觉任务，无需任务特定调整，在性能和效率方面都优于现有方法，为3D视觉任务提供了统一的解决方案。
...
TL;DR: 提出了一种仅使用相机的3D重建管道，通过3D高斯泼溅从多视角图像重建场景，结合语义材料掩码和物理材料属性，实现与LiDAR-相机融合相当的传感器仿真精度，同时消除了硬件复杂性和校准需求。


<details>
  <summary>Details</summary>
Motivation: 传统LiDAR方法虽然几何精度高但缺乏语义和纹理信息，LiDAR-相机融合方法需要复杂校准且对玻璃等材料处理不佳。需要一种既能保持高保真度又简化硬件需求的方法。

Method: 使用多视角图像通过3D高斯泼溅进行场景重建，通过视觉模型提取语义材料掩码，将高斯表示转换为带材料标签的网格表面，并分配基于物理的材料属性用于传感器仿真。

Result: 在仪器化测试车辆内部数据集上验证，利用LiDAR作为反射率真值验证，同时使用图像相似性指标评估，实现了与LiDAR-相机融合相当的传感器仿真保真度。

Conclusion: 该方法成功结合了照片级真实感重建与基于物理的材料分配，在保持传感器仿真精度的同时，显著简化了硬件设置和校准流程。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉搜索方法局限于静态图像，忽略了物理体现与3D世界的互动。需要开发能够像人类一样高效进行视觉搜索的具身智能体，同时避免真实世界硬件的限制。

Method: 提出人形视觉搜索框架，构建H* Bench基准测试包含交通枢纽、大型零售空间等复杂场景，使用后训练技术增强开源模型Qwen2.5-VL的性能。

Result: 顶级专有模型在物体和路径搜索中成功率仅约30%。通过后训练，Qwen2.5-VL的物体搜索成功率从14.83%提升至47.38%，路径搜索从6.44%提升至24.94%。路径搜索的难度明显更高。

Conclusion: 研究展示了构建高效视觉搜索智能体的可行路径，但量化了构建能够无缝融入人类日常生活的MLLM智能体所面临的巨大挑战，特别是路径搜索需要更复杂的空间常识推理能力。
...
TL;DR: GS-Checker是一种用于定位3D高斯泼溅(3DGS)模型中篡改区域的新方法，通过集成3D篡改属性和对比机制来检测恶意操作。
...
Method: 在3D高斯参数中集成3D篡改属性来标记高斯是否被篡改；设计3D对比机制比较高斯间关键属性的相似性；引入循环优化策略来精炼篡改属性。

Result: 广泛的实验结果证明了该方法在定位篡改3DGS区域方面的有效性。

Conclusion: GS-Checker能够准确定位3DGS模型中的篡改区域，且无需昂贵的3D标签进行监督。
...
TL;DR: BoxPromptIML是一个弱监督图像篡改定位框架，通过粗粒度区域标注策略和基于SAM的知识蒸馏，在降低标注成本的同时实现精细定位，性能可与全监督方法媲美。


<details>
  <summary>Details</summary>
Motivation: 解决图像篡改定位中标注成本与定位精度之间的根本权衡问题。全监督方法依赖密集像素级标注成本高，而弱监督方法基于图像级标签缺乏精确空间定位。

Method: 提出粗粒度区域标注策略降低标注成本；设计轻量级学生模型通过SAM教师模型进行知识蒸馏；基于人类潜意识记忆机制设计双引导特征融合模块，动态结合原型模式与实时观察线索。

Result: 在分布内和分布外数据集上的广泛实验表明，BoxPromptIML优于或可与全监督模型相媲美，同时保持强泛化性、低标注成本和高效部署特性。
...
### [114] [StableTrack: Stabilizing Multi-Object Tracking on Low-Frequency Detections](https://arxiv.org/abs/2511.20418)
*Matvei Shelukhan,Timur Mamedov,Karina Kvanchiani*

Main category: cs.CV

TL;DR: StableTrack是一种新颖的多目标跟踪方法，通过两阶段匹配策略和Bbox-Based距离来提升低频检测下的跟踪质量，在保持标准基准性能的同时显著改善低频检测场景的跟踪效果。


<details>
  <summary>Details</summary>
Motivation: 当前多目标跟踪方法主要在视频流的每一帧中跟踪对象，在计算资源有限的情况下几乎无法运行。为了解决这个问题，需要开发能够在低频检测条件下稳定跟踪质量的方法。

Method: 提出两阶段匹配策略来改进低频检测之间的跨帧关联；引入新颖的Bbox-Based距离替代传统的Mahalanobis距离，使用Re-ID模型有效匹配对象；将视觉跟踪集成到卡尔曼滤波器和整体跟踪流程中。

Result: 在低频检测情况下优于当前最先进的跟踪器，在MOT17-val上1Hz时HOTA提升11.6%，同时在标准MOT17、MOT20和DanceTrack基准测试中保持与最佳方法相当的性能。

Conclusion: StableTrack方法能够有效解决低频检测条件下的多目标跟踪问题，在保持标准性能的同时显著提升低频场景的跟踪质量，为资源受限环境提供了可行的解决方案。
...
TL;DR: VGGTFace是一个自动化的面部几何重建方法，利用3D基础模型VGGT从多视角图像重建拓扑一致的面部几何，通过结合Pixel3DMM注入拓扑信息，并提出拓扑感知的捆绑调整策略。
...
Method: 利用VGGT基础模型获取点图表示，通过Pixel3DMM注入拓扑信息生成带拓扑的点云，提出拓扑感知捆绑调整策略融合点云。

Result: 在单张NVIDIA RTX 4090上16个视图仅需10秒完成高质量重建，在基准测试中达到最先进结果，对野外数据具有出色的泛化能力。

Conclusion: VGGTFace成功实现了从多视角图像自动重建拓扑一致面部几何，结合了VGGT的泛化能力和拓扑信息注入，实现了高效高质量的重建。
...
TL;DR: FREE是一个针对扩散变换器(DiTs)的加速框架，通过特征级自回归和并行验证实现无损加速，在ImageNet-512²上达到2.25倍加速效果。
...
Method: 提出FREE框架：1）利用DiTs顶层特征的时间一致性和语义抽象性，使用轻量级草稿器进行特征级自回归；2）引入不确定性引导的松弛策略(FREE relax)，根据不确定性水平动态调整接受概率。

Result: 在ImageNet-512²上，FREE实现1.86倍加速，FREE (relax)进一步达到2.25倍加速，同时保持生成质量的高感知和定量保真度。

Conclusion: FREE通过特征级自回归和不确定性引导的松弛策略，有效解决了DiTs加速中的草稿准确性不足问题，实现了显著的无损加速效果。
...
<details>
  <summary>Details</summary>
Motivation: 多ID定制化相比单ID定制化面临两大挑战：一是推理时出现复制粘贴问题导致质量下降，二是文本控制性差，生成结果与输入文本不一致。

Method: 提出ID解耦交叉注意力机制，将不同ID嵌入注入对应图像区域；引入局部提示、深度引导空间控制和扩展自注意力三种策略增强生成可控性。

Result: 在构建的IDBench基准测试中，定性和定量结果均表明MultiID能有效解决上述两个挑战，性能与基于训练的方法相当甚至更好。

Conclusion: MultiID方法无需训练即可实现高质量的多ID定制化，在保持身份特征的同时提高了文本控制能力。
...
TL;DR: 提出了轨迹反向一致性模型（TBCM），通过直接从教师模型的生成轨迹中提取潜在表示，消除了对外部训练数据的依赖，显著提高了扩散模型时间步蒸馏的效率和简洁性。
...
Method: 采用自包含的蒸馏范式，从教师模型的生成轨迹中提取潜在表示，无需VAE编码和大规模数据集，通过轨迹提取的样本自然桥接训练和推理之间的分布差距。

Result: 在MJHQ-30k数据集上，一步生成达到6.52 FID和28.08 CLIP分数，相比Sana-Sprint减少约40%的训练时间，并节省大量GPU内存。

Conclusion: TBCM在保持质量的同时实现了卓越的效率，揭示了连续时间一致性蒸馏中的扩散-生成空间差异，并分析了采样策略对蒸馏性能的影响。
...
### [119] [Quantifying Modality Contributions via Disentangling Multimodal Representations](https://arxiv.org/abs/2511.19470)
*Padegal Amit,Omkar Mahesh Kashyap,Namitha Rayasam,Nidhi Shekhar,Surabhi Narayan*

Main category: cs.LG

TL;DR: 提出了一个基于部分信息分解(PID)的框架，通过将内部嵌入中的预测信息分解为独特、冗余和协同成分来量化模态贡献，并开发了基于迭代比例拟合程序(IPFP)的算法进行可扩展的推理分析。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖基于准确性的方法，将移除模态后的性能下降解释为其影响力，但这种结果驱动指标无法区分模态是固有信息性的还是仅通过与其他模态交互才产生价值，特别是在跨注意力架构中模态相互影响表示的情况下。

Method: 基于部分信息分解(PID)框架，将预测信息分解为独特、冗余和协同成分；开发基于迭代比例拟合程序(IPFP)的算法，无需重新训练即可计算层和数据集级别的贡献。

Result: 提供了一个原则性的、表示级别的多模态行为视图，比基于结果的指标提供更清晰和可解释的见解。

Conclusion: 该框架能够量化模态贡献，区分模态的固有信息性和交互价值，为多模态模型分析提供了更精确和可解释的方法。
...
### [120] [PrefixGPT: Prefix Adder Optimization by a Generative Pre-trained Transformer](https://arxiv.org/abs/2511.19472)
*Ruogu Ding,Xin Ning,Ulf Schlichtmann,Weikang Qian*

Main category: cs.LG

TL;DR: PrefixGPT是一个基于GPT的生成式预训练Transformer模型，能够从零开始直接生成优化的前缀加法器，在保证设计合法性的同时显著提升面积延迟积性能。


<details>
  <summary>Details</summary>
Motivation: 前缀加法器在计算密集型应用中广泛使用，但设计优化的前缀加法器具有挑战性，因为设计规则严格且设计空间呈指数级增长。

Method: 将加法器拓扑表示为二维坐标序列，在生成过程中应用合法性掩码确保每个设计都有效构建；采用定制化的仅解码器Transformer架构，先在随机合成的有效前缀加法器语料库上进行预训练学习设计规则，然后微调以在优化设计质量的设计空间中导航。

Result: 相比现有工作，PrefixGPT不仅找到了一个面积延迟积(ADP)改善7.7%的新最优设计，而且展现出优越的探索质量，将平均ADP降低了高达79.1%。
...
### [121] [WavefrontDiffusion: Dynamic Decoding Schedule or Improved Reasoning](https://arxiv.org/abs/2511.19473)
...
<details>
  <summary>Details</summary>
Motivation: 主流去噪策略（标准扩散和块扩散）存在局限性：标准扩散容易过早结束序列，块扩散的刚性结构会破坏语义连贯性。需要一种更自然的自适应调度方法。

Method: 提出WavefrontDiffusion方法，动态扩展活动标记波前，从已确定位置向外传播，遵循语义结构的自然流动。
...
### [122] [Exploiting the Experts: Unauthorized Compression in MoE-LLMs](https://arxiv.org/abs/2511.19480)
*Pinaki Prasad Guha Neogi,Ahmad Mohammadshirazi,Dheeraj Kulshrestha,Rajiv Ramnath*

Main category: cs.LG

TL;DR: 本文系统研究了MoE-LLMs在任务特定使用下的可修剪性，揭示了专家修剪带来的知识损失-恢复权衡，并提出了防御策略以防止未经授权的模型压缩和微调。
...
Method: 开发专家归因框架识别对特定任务最关键的专家子集，评估修剪和重新对齐这些专家的性能权衡，使用主动学习驱动的微调方法。

Result: 发现关键的知识损失-恢复权衡：虽然可以隔离某些专家来保持任务准确性，但如果没有针对性的重新对齐，会出现显著的性能下降。
...
### [123] [Quality analysis and evaluation prediction of RAG retrieval based on machine learning algorithms](https://arxiv.org/abs/2511.19481)
*Ruoxin Zhang,Zhizhao Wen,Chao Wang,Chenchen Tang,Puyang Xu,Yifan Jiang*

Main category: cs.LG

TL;DR: 本文提出了一种基于特征工程和粒子群优化的XGBoost机器学习回归模型，用于解决检索增强生成系统中检索模块的性能瓶颈问题，特别是在处理表格特征方面。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的快速发展，检索增强生成技术因其能够整合外部知识来提高输出准确性而被广泛应用。然而，系统性能高度依赖于检索模块的质量，如果检索结果与用户需求相关性低或包含噪声信息，将直接导致生成内容失真。

Method: 提出XGBoost机器学习回归模型，结合特征工程和粒子群优化算法。通过相关性分析发现文档相关性与答案质量呈正相关，语义相似性、冗余性与多样性之间存在权衡关系。

Result: 实验结果表明，VMD PSO BiLSTM模型在所有评估指标上均优于决策树、AdaBoost等对比模型，具有更低的MSE、RMSE、MAE和MAPE值，以及更高的R2值，表明其预测精度、稳定性和数据解释能力更突出。

Conclusion: 该成果为优化检索质量和改进RAG系统的生成效果提供了有效路径，在推动相关技术实施和应用方面具有重要价值。
...
### [124] [OmniTFT: Omni Target Forecasting for Vital Signs and Laboratory Result Trajectories in Multi Center ICU Data](https://arxiv.org/abs/2511.19485)
*Wanzhe Xu,Yutong Dai,Yitao Yang,Martin Loza,Weihang Zhang,Yang Cui,Xin Zeng,Sung Joon Park,Kenta Nakai*

Main category: cs.LG

TL;DR: OmniTFT是一个基于时间融合变换器的深度学习框架，用于联合预测ICU中的高频生命体征和稀疏采样的实验室结果，通过四种创新策略提升性能，在多个数据集上取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: ICU中的生命体征存在噪声和快速波动，实验室结果存在缺失值、测量延迟和设备特异性偏差，这使得综合预测极具挑战性。需要开发能够统一建模多种异质临床目标的框架。

Method: 基于时间融合变换器(TFT)构建OmniTFT框架，采用四种策略：滑动窗口均衡采样平衡生理状态、频率感知嵌入收缩稳定罕见类别表示、分层变量选择引导模型注意力、影响对齐注意力校准增强鲁棒性。

Result: 在MIMIC-III、MIMIC-IV和eICU数据集上的预测任务中，OmniTFT对生命体征和实验室结果都实现了显著的性能提升，注意力模式可解释且与已知病理生理学一致。

Conclusion: OmniTFT减少了对目标特定架构和广泛特征工程的依赖，实现了多个异质临床目标的统一建模，同时保持了跨机构泛化能力，在临床护理中具有定量决策支持的潜在实用性。
...
### [125] [Efficient Inference Using Large Language Models with Limited Human Data: Fine-Tuning then Rectification](https://arxiv.org/abs/2511.19486)
*Lei Wang,Zikun Ye,Jinglong Zhao*

Main category: cs.LG

TL;DR: 本文提出了一种结合微调和修正的框架，通过优化分配有限标记样本来提升大语言模型在市场研究和社会科学应用中的表现。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能的发展，大语言模型在市场研究和社会科学应用中显示出生成类人响应的潜力。现有方法主要通过微调或修正来提升模型性能，但如何最优结合这两种方法并分配有限样本资源仍待研究。

Method: 开发了一个结合微调和修正的框架，提出以最小化预测误差方差作为微调目标（这对下游修正阶段更优），并基于经验缩放定律开发数据驱动方法来最优分配样本到两个阶段。

Result: 实证分析验证了该框架的有效性，相比单独使用微调或修正，该框架在估计和推断性能方面均有提升。

Conclusion: 提出的结合微调和修正的框架能够更有效地利用有限样本资源，显著提升大语言模型在市场研究和社会科学应用中的性能表现。
...
### [126] [The Generalized Proximity Forest](https://arxiv.org/abs/2511.19487)
...
<details>
  <summary>Details</summary>
Motivation: 随机森林邻近性在多种监督学习任务中具有实用性，但其效用依赖于随机森林模型的成功，而随机森林并非所有场景下的理想模型。需要将邻近性扩展到更广泛的基于距离的机器学习场景。

Method: 引入广义邻近森林模型，将随机森林邻近性扩展到所有基于距离的监督机器学习场景；开发了回归任务的变体；将广义邻近森林模型作为元学习框架，扩展监督插补能力到任何预训练分类器。

Result: 实验证明广义邻近森林模型相比随机森林模型和k近邻模型具有独特优势。

Conclusion: 广义邻近森林模型成功扩展了随机森林邻近性的应用范围，为基于距离的监督机器学习提供了更通用的框架，并在实验中显示出优于传统方法的性能。
...
### [127] [OpenCML: End-to-End Framework of Open-world Machine Learning to Learn Unknown Classes Incrementally](https://arxiv.org/abs/2511.19491)
...
Conclusion: 该模型能够在开放连续学习环境中有效扩展对数据的理解并随时间改进，实现了持续学习能力。
...
### [128] [A Systematic Study of Compression Ordering for Large Language Models](https://arxiv.org/abs/2511.19495)
*Shivansh Chhawri,Rahul Mahadik,Suparna Rooj*

Main category: cs.LG

TL;DR: 本文系统研究了知识蒸馏、结构化剪枝和低比特量化三种LLM压缩技术的独立效果和组合序列，发现在Qwen2.5 3B模型中，P-KD-Q（剪枝-知识蒸馏-量化）序列能获得最佳平衡，达到3.68倍压缩比同时保持强指令跟随和语言理解能力。


<details>
  <summary>Details</summary>
Motivation: LLM需要大量计算资源，模型压缩对受限环境部署至关重要。虽然各压缩技术独立效果已有研究，但它们的交互作用和最优序列仍不明确。

Method: 在Qwen2.5 3B模型上系统评估多种压缩流水线，包括单技术和三技术序列，使用困惑度、G-Eval、清晰度、提示对齐和压缩比作为指标。

Result: 量化提供最大的独立压缩，剪枝引入中等质量下降。技术顺序显著影响最终模型质量：P-KD-Q序列表现最佳，而早期应用量化的流水线因不可逆信息损失导致严重性能下降。

Conclusion: 为在资源受限环境中部署LLM，设计有效的、顺序感知的压缩流水线至关重要，P-KD-Q序列提供了最佳平衡方案。
...
### [129] [Xmodel-2.5: 1.3B Data-Efficient Reasoning SLM](https://arxiv.org/abs/2511.19496)
*Yang Liu,Xiaolong Zhong,Ling Jiang*

Main category: cs.LG

TL;DR: Xmodel-2.5是一个13亿参数的小型语言模型，采用μP训练方法，通过从AdamW切换到Muon优化器在衰减阶段提升了推理性能，使用FP8混合精度训练平衡精度和吞吐量。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然具有强大的推理和工具使用能力，但计算需求大，不适合边缘或成本敏感部署。

Method: 采用最大更新参数化(μP)训练方法，使用14万亿token的Warmup-Stable-Decay课程学习，在衰减阶段从AdamW切换到Muon优化器，采用FP8混合精度训练。

Result: 在衰减阶段从AdamW切换到Muon优化器使13个任务的推理平均性能提升了4.58%，同时保持其他超参数不变。

Conclusion: Xmodel-2.5作为drop-in agent core，验证了早期AdamW稳定性与后期Muon锐化相结合可以提升下游性能，所有检查点、配方和评估代码均已开源。
...
### [130] [Hierarchical Dual-Strategy Unlearning for Biomedical and Healthcare Intelligence Using Imperfect and Privacy-Sensitive Medical Data](https://arxiv.org/abs/2511.19498)
*Yi Zhang,Tianxiang Xu,Zijian Li,Chao Zhang,Kunyu Zhang,Zhan Gao,Meinuo Li,Xiaohan Zhang,Qichao Qi,Bing Chen*

Main category: cs.LG

TL;DR: 本文提出了一个分层双策略框架，用于选择性知识遗忘，在医疗领域精确移除专业知识的同时保留基本医学能力，仅需修改0.1%的参数即可实现82.7%的遗忘率和88.5%的知识保留。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医疗环境中存在隐私风险，特别是训练数据记忆问题，需要解决专业知识移除与基本能力保留的平衡问题。
...
### [131] [Beyond Binary Classification: A Semi-supervised Approach to Generalized AI-generated Image Detection](https://arxiv.org/abs/2511.19499)
*Hong-Hanh Nguyen-Le,Van-Tuan Tran,Dinh-Thuc Nguyen,Nhien-An Le-Khac*

Main category: cs.LG

TL;DR: 本文提出TriDetect检测器，通过发现"假"类中的潜在架构模式来增强跨生成器泛化能力，解决GAN和扩散模型检测器无法跨架构边界泛化的问题。


<details>
  <summary>Details</summary>
Motivation: 当前取证检测器在跨生成器泛化方面存在重大挑战，特别是跨越架构边界（如从GAN到扩散模型）时失效。这种差距源于不同架构产生的伪影存在根本性差异。

Method: 提出TriDetect半监督方法，通过Sinkhorn-Knopp算法进行平衡聚类分配和跨视图一致性机制，学习基本架构差异。

Result: 在两个标准基准和三个真实数据集上评估，与13个基线方法比较，证明了其对未见生成器的泛化能力。

Conclusion: TriDetect通过发现潜在架构模式有效解决了跨生成器检测的泛化问题，为数字媒体真实性验证提供了新思路。
...
### [132] [Position: The Complexity of Perfect AI Alignment -- Formalizing the RLHF Trilemma](https://arxiv.org/abs/2511.19504)
*Subramanyam Sahoo,Aman Chadha,Vinija Jain,Divya Chaudhary*

Main category: cs.LG

TL;DR: 本文提出了对齐三难困境：RLHF系统无法同时实现代表性、计算可扩展性和鲁棒性。通过复杂性理论分析证明，实现全球规模的代表性和鲁棒性需要超多项式计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 解决RLHF实践中存在的安全性与公平性冲突、扩展到多样化群体的计算困难以及鲁棒性导致多数偏见放大的问题。

Method: 采用复杂性理论分析，结合统计学习理论和鲁棒优化，证明对齐三难困境的理论界限。

Result: 证明同时实现代表性（ε≤0.01）和鲁棒性（δ≤0.001）需要Ω(2^{d_context})次操作，这是超多项式复杂度。当前RLHF实现通过牺牲代表性来解决这一困境。
...
### [133] [Row-stochastic matrices can provably outperform doubly stochastic matrices in decentralized learning](https://arxiv.org/abs/2511.19513)
*Bing Liu,Boao Kong,Limin Lu,Kun Yuan,Chengcheng Zhao*

Main category: cs.LG

TL;DR: 本文重新审视了去中心化学习中处理异构节点权重的两种策略：将权重嵌入局部损失以保持双重随机矩阵，或保持原始损失并使用行随机矩阵。作者建立了加权希尔伯特空间框架，发现行随机矩阵在该几何中成为自伴算子，而双重随机矩阵则不会，这产生了额外的惩罚项，放大了共识误差并减缓收敛。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明两种策略对全局损失的期望下降方向相同，但尚不清楚欧几里得空间保证是否紧密，以及它们行为的根本差异。本文旨在澄清这些差异，并理解收敛速度差异的来源。

Method: 开发了加权希尔伯特空间框架L^2(λ;ℝ^d)，在该几何中分析两种策略的收敛行为。使用Rayleigh商和Loewner阶特征值比较来推导拓扑条件。

Result: 在加权希尔伯特空间中，行随机矩阵成为自伴算子而双重随机矩阵不会，这产生了额外的惩罚项，放大了共识误差并减缓收敛。收敛差异不仅来自谱间隙，还来自这些惩罚项。推导了行随机设计即使具有较小谱间隙也能更快收敛的充分条件。

Conclusion: 通过建立加权希尔伯特空间框架，揭示了两种权重处理策略收敛行为的根本差异。行随机矩阵的自伴性质导致了额外的共识误差惩罚，而双重随机策略避免了这一问题。研究结果为实际拓扑设计提供了指导原则。
...
### [134] [Automating Deception: Scalable Multi-Turn LLM Jailbreaks](https://arxiv.org/abs/2511.19517)
*Adarsh Kumarappan,Ananya Mujoo*

Main category: cs.LG

TL;DR: 本文提出了一个自动化生成心理驱动的多轮越狱数据集的新方法，评估了不同LLM在多轮对话攻击中的安全漏洞，发现GPT系列模型对对话历史特别敏感，而Gemini 2.5 Flash表现出卓越的抵抗力。
...
Method: 开发自动化流水线，将FITD技术系统化为可复现模板，创建包含1500个场景的基准数据集，评估7个模型在多轮和单轮条件下的表现。

Result: GPT系列模型对对话历史极其敏感，攻击成功率最多增加32个百分点；Gemini 2.5 Flash几乎完全免疫这些攻击；Claude 3 Haiku表现出强但不完美的抵抗力。
...
### [135] [When Should Neural Data Inform Welfare? A Critical Framework for Policy Uses of Neuroeconomics](https://arxiv.org/abs/2511.19548)
*Yiven,Zhu*

Main category: cs.LG

TL;DR: 本文提出了一个基于模型的框架，分析神经数据在何种条件下可以合法地用于政策福利判断，而不仅仅是描述行为。该框架连接神经信号、计算决策模型和规范性福利标准三个层次，并开发了神经经济福利推断检查表。


<details>
  <summary>Details</summary>
Motivation: 随着神经经济学的发展，政策和商业行为体越来越多地引用神经数据来为家长式监管、"基于大脑"的干预措施和新的福利措施提供依据。本文旨在探讨神经数据在什么条件下可以合法地为政策福利判断提供信息，而不仅仅是描述行为。

Method: 开发了一个非经验性的、基于模型的框架，该框架连接三个层次：神经信号、计算决策模型和规范性福利标准。在演员-评论家强化学习模型中，形式化了从神经活动到潜在价值和预测误差，再到福利声明的推断路径。

Result: 研究表明，只有当神经-计算映射得到充分验证、决策模型识别出"真实"利益与情境依赖的错误、福利标准被明确指定和辩护时，神经证据才能约束福利判断。应用该框架到成瘾、神经营销和环境政策中，推导出了神经经济福利推断检查表。

Conclusion: 分析将大脑和人工智能体视为价值学习系统，同时表明内部奖励信号（无论是生物的还是人工的）都是计算量，如果没有明确的规范性模型，就不能被视为福利衡量标准。
...
### [136] [Online Sparse Feature Selection in Data Streams via Differential Evolution](https://arxiv.org/abs/2511.19555)
*Ruiyang Xu*

Main category: cs.LG

TL;DR: 本文提出了一种新颖的在线差分进化稀疏特征选择方法(ODESFS)，用于处理高维流数据中的特征选择问题，通过潜在因子分析进行缺失值填补和差分进化进行特征重要性评估，在六个真实数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的在线稀疏流特征选择方法在特征评估方面存在显著局限性，导致性能下降，需要改进特征选择策略以提升流数据处理效果。
...
Conclusion: ODESFS方法通过改进的缺失值填补和特征评估机制，有效提升了高维流数据特征选择的性能。
...
### [137] [Merging without Forgetting: Continual Fusion of Task-Specific Models via Optimal Transport](https://arxiv.org/abs/2511.19561)
...
TL;DR: OTMF提出了一种基于最优传输理论的模型融合框架，通过语义几何对齐解决参数插值带来的分布偏移问题，支持持续融合范式，在多个视觉和语言基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有模型融合方法主要依赖权重空间的参数插值，但这会导致特征空间的显著分布偏移并削弱任务特定知识。需要一种新方法来保持任务特定知识的同时实现有效融合。

Method: OTMF使用最优传输理论发现应用于任务向量的共同掩码，通过最优传输计划对齐任务特定模型的语义几何。这些掩码选择性地提取可转移和任务无关组件，同时保留每个任务的独特结构身份。

Result: 在多个视觉和语言基准测试上的综合实验表明，OTMF在准确性和效率方面均达到最先进性能。

Conclusion: OTMF通过最优传输理论有效解决了模型融合中的分布偏移问题，支持持续融合，具有重要的实践和理论价值。
...
### [138] [ModHiFi: Identifying High Fidelity predictive components for Model Modification](https://arxiv.org/abs/2511.19566)
...
<details>
  <summary>Details</summary>
Motivation: 现有模型修改技术通常需要梯度或真实标签，在计算资源有限的情况下不可行。本文研究如何在没有梯度或损失函数访问的情况下识别模型关键组件。

Method: 提出子集保真度指标，利用组件子集的局部重建行为量化其全局重要性。基于此开发ModHiFi算法，包括用于结构化剪枝的ModHiFi-P和用于类别遗忘的ModHiFi-U。

Result: ModHiFi-P在ImageNet模型上比现有技术快11%，在语言模型上表现有竞争力；ModHiFi-U在CIFAR-10上无需微调即可实现完全遗忘，在Swin Transformers上表现良好。

Conclusion: 研究表明局部重建误差可以线性约束全局重建误差，子集保真度是有效的模型组件重要性度量，ModHiFi为无训练数据访问的模型修改提供了可行方案。
...
### [139] [An Invariant Latent Space Perspective on Language Model Inversion](https://arxiv.org/abs/2511.19569)
*Wentao Ye,Jiaqi Hu,Haobo Wang,Xinpeng Ti,Zhiqing Xiao,Hao Chen,Liyao Li,Lei Feng,Sai Wu,Junbo Zhao*

Main category: cs.LG

TL;DR: 本文提出语言模型逆向攻击（LMI）威胁用户隐私和系统安全，基于不变潜在空间假设（ILSH）开发了Inv^2A方法，通过轻量级逆向编码器从输出恢复隐藏提示，在9个数据集上表现优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 语言模型逆向攻击（LMI）对用户隐私和系统安全构成实际威胁，需要开发有效的攻击方法来评估和提升防御能力。

Method: 提出不变潜在空间假设（ILSH），将LLM视为不变解码器，仅学习轻量级逆向编码器将输出映射到去噪伪表示。采用两阶段训练：对比对齐（源不变性）和监督强化（循环不变性）。

Result: 在9个数据集上，Inv^2A比基线方法平均BLEU得分提高4.77%，同时减少对大型逆向语料库的依赖。分析表明现有防御措施保护有限。

Conclusion: Inv^2A方法在语言模型逆向攻击方面表现优异，突显了需要更强防御策略的必要性。
...
### [140] [Learning Massively Multitask World Models for Continuous Control](https://arxiv.org/abs/2511.19584)
*Nicklas Hansen,Hao Su,Xiaolong Wang*

Main category: cs.LG

TL;DR: 本文提出了一个包含200个多样化任务的基准测试，并介绍了Newt——一个语言条件化的多任务世界模型，通过演示预训练和在线交互联合优化，在多任务性能、数据效率和开放回路控制方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 通用控制需要能够在多种任务和体现形式上工作的智能体，但目前连续控制的强化学习研究主要局限于单任务或离线模式，缺乏在线强化学习的可扩展性研究。

Method: 首先在演示数据上进行预训练以获取任务感知表示和动作先验，然后通过在线交互在所有任务上进行联合优化。

Result: Newt在多任务性能和数据效率方面优于强基线方法，展现出强大的开放回路控制能力，并能快速适应未见过的任务。

Conclusion: 该研究表明在线强化学习可以扩展到数百个任务，为通用控制智能体的开发提供了新的可能性。
...
### [141] [Lower Complexity Bounds for Nonconvex-Strongly-Convex Bilevel Optimization with First-Order Oracles](https://arxiv.org/abs/2511.19656)
*Kaiyi Ji*
...
<details>
  <summary>Details</summary>
Motivation: 尽管双层优化的上界保证已被广泛研究，但由于双层结构的复杂性，下界研究的进展有限。本文旨在填补这一空白，为双层优化建立非平凡的下界。

Method: 开发新的困难实例，在光滑非凸-强凸设置下，分别针对确定性和随机一阶oracle模型进行分析。

Result: 在确定性情况下，任何一阶零保持算法至少需要Ω(κ^{3/2}ε^{-2})次oracle调用才能找到ε精确的稳定点；在随机情况下，至少需要Ω(κ^{5/2}ε^{-4})次随机oracle调用。
...
### [142] [Demystifying Diffusion Objectives: Reweighted Losses are Better Variational Bounds](https://arxiv.org/abs/2511.19664)
*Jiaxin Shi,Michalis K. Titsias*

Main category: cs.LG

TL;DR: 本文提出了对扩散模型中广泛使用的重加权损失的新理论解释，通过构建时间相关的变分下界级联来改进标准证据下界，从而减少数据模型KL散度。该方法适用于连续高斯扩散和掩码扩散模型，在掩码扩散中显著提升了像素空间图像建模性能。


<details>
  <summary>Details</summary>
Motivation: 为扩散模型中广泛使用的重加权损失提供理论依据，改进标准证据下界，降低数据模型KL散度，提升生成模型性能。

Method: 构建时间相关的变分下界级联，结合这些下界产生重加权目标，适用于连续高斯扩散和掩码扩散模型。

Result: 在掩码扩散中显著提升了像素空间图像建模性能，样本质量接近连续扩散模型水平，并为掩码图像模型中简单加权方案提供了理论依据。

Conclusion: 提出的理论框架为扩散模型重加权损失提供了新的理论解释，显著提升了掩码扩散模型的性能，并为现有加权方案提供了理论支持。
...
### [143] [TiCT: A Synthetically Pre-Trained Foundation Model for Time Series Classification](https://arxiv.org/abs/2511.19694)
...
<details>
  <summary>Details</summary>
Motivation: 时间序列数据普遍存在，但标记数据成本高昂。现有大规模时间序列模型主要关注预测任务，缺乏无需微调、支持上下文学习的通用分类基础模型。
...
Conclusion: TiCT证明了仅使用合成数据预训练的模型能够通过上下文学习在时间序列分类任务中取得优异表现，为开发通用时间序列基础模型提供了可行路径。
...
<details>
  <summary>Details</summary>
Motivation: 传统机器学习模型在主动学习中存在冷启动问题和领域特定特征工程限制，需要更通用的解决方案。

Method: 开发基于LLM的主动学习框架，采用迭代少样本设置，探索两种提示策略：简洁数值输入和扩展描述性文本输入。

Result: 在所有数据集上，LLM-AL将到达最佳候选所需的实验次数减少70%以上，性能始终优于传统ML模型，进行更广泛探索性搜索。
...
TL;DR: DISCO是一个开源的分布式协作学习平台，允许非技术用户在不共享原始数据或编程知识的情况下协作构建机器学习模型。该平台在浏览器中本地训练模型，支持联邦和去中心化学习范式，并提供隐私保护、模型个性化等功能。


<details>
  <summary>Details</summary>
Motivation: 数据共享存在隐私、知识产权和法律限制等问题，导致预测模型的统计能力分散，并造成资源不平等问题。需要开发一个工具让用户能够协作构建机器学习模型而无需共享原始数据。

Method: DISCO是一个基于Web的分布式协作学习平台，在浏览器中本地训练模型，支持跨平台使用。采用模块化设计，提供联邦学习和去中心化学习选择，支持不同的隐私保证级别和权重聚合策略。

Result: 开发了一个开源平台，代码库位于https://github.com/epfml/disco，展示界面在https://discolab.ai。该工具使非技术用户能够协作训练机器学习模型，同时保护数据隐私。

Conclusion: DISCO平台成功解决了数据共享的限制问题，通过分布式协作学习使机器学习模型训练更加民主化，同时保证了数据隐私和模型个性化需求。
...
### [146] [When +1% Is Not Enough: A Paired Bootstrap Protocol for Evaluating Small Improvements](https://arxiv.org/abs/2511.19794)
*Wenzhang Du*

Main category: cs.LG

TL;DR: 本文提出了一种保守的评估协议，用于在有限计算预算下验证机器学习基准测试中1-2个百分点的微小改进是否具有统计显著性，避免将噪声误认为算法进步。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习论文中报告的1-2个百分点改进往往缺乏不确定性估计和显著性检验，这些微小增益对随机种子、数据排序和实现细节高度敏感，难以区分是真实算法进步还是噪声。

Method: 提出基于配对多种子运行、偏差校正加速(BCa)自助法置信区间和符号翻转置换检验的简单评估协议，该协议设计保守，旨在防止过度声称。

Result: 在CIFAR-10、CIFAR-10N和AG News数据集上的实验显示，单次运行和非配对t检验经常将0.6-2.0个百分点的改进误判为显著，而使用三个种子的配对协议在这些设置下从未声明显著性。

Conclusion: 对于有限预算下的小幅改进，这种保守的评估方法是一个更安全的默认选择，能够有效防止过度解读微小增益。
...
### [147] [Terminal Velocity Matching](https://arxiv.org/abs/2511.19797)
*Linqi Zhou,Mathias Parger,Ayaan Haque,Jiaming Song*

Main category: cs.LG

TL;DR: TVM是一种广义的流匹配方法，支持高保真的一步和少步生成建模，通过在终端时间而非初始时间进行正则化，并在ImageNet数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 传统的流匹配方法在初始时间进行正则化，TVM旨在通过在终端时间进行正则化来改进生成模型，特别是针对缺乏Lipschitz连续性的扩散变换器。

Method: TVM建模任意两个扩散时间步之间的转换，引入最小架构变化以实现稳定的单阶段训练，并开发了支持Jacobian-Vector Products反向传播的融合注意力核。

Result: 在ImageNet-256x256上，TVM单步评估（NFE）达到3.29 FID，4步评估达到1.99 FID；在ImageNet-512x512上，单步达到4.32 FID，4步达到2.94 FID，创下了从头训练的一步/少步模型的最先进性能。

Conclusion: TVM通过终端速度匹配和架构优化，实现了高效的高质量生成建模，在少步评估下达到了最先进的性能。
...
### [148] [Cisco Time Series Model Technical Report](https://arxiv.org/abs/2511.19841)
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够处理多分辨率输入的时间序列基础模型，特别是在可观测性领域提升预测性能，同时保持通用预测基准的竞争力。

Method: 对流行的仅解码器时间序列模型(TimesFM)进行架构创新，使其能够接受多分辨率输入，构建多分辨率仅解码器模型。

Result: 模型在可观测性数据集上实现卓越性能，在通用预测基准(GIFT-Eval)上保持相似性能，多分辨率结构使模型在长上下文输入上预测更准确。

Conclusion: 多分辨率架构创新成功提升了时间序列模型在可观测性领域的性能，同时保持了通用预测能力，为时间序列基础模型的发展提供了新方向。
...
### [149] [Accelerating Wireless Distributed Learning via Hybrid Split and Federated Learning Optimization](https://arxiv.org/abs/2511.19851)
...
Method: 首先分析收敛性，揭示学习模式和批次大小之间的相互作用；然后制定延迟最小化问题，提出两阶段解决方案：使用块坐标下降法求解松弛问题获得局部最优解，随后通过舍入算法恢复整数批次大小以获得接近最优性能。

Result: 实验结果表明，与现有方法相比，该方法显著加速了达到目标精度的收敛过程。

Conclusion: 通过联合优化学习模式、批次大小和资源分配，可以有效加速HSFL的收敛速度，实现更好的性能平衡。
...
Result: 在多伦多网约车数据上的结果显示，提出的FACT模型获得了最高的时间依赖性C指数和最低的Brier分数，优于经典和深度学习生存模型。
...
TL;DR: EfficientXpert是一个轻量级领域剪枝框架，通过传播感知剪枝准则和高效适配器更新算法，在LoRA微调过程中将通用预训练模型一步转换为稀疏的领域适应专家模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在资源受限环境中的部署面临挑战，现有压缩方法要么跨领域泛化能力差，要么开销过高。

Method: 结合传播感知剪枝准则（Foresight Mask）和高效适配器更新算法（Partial Brain Surgeon），集成到LoRA微调过程中。

Result: 在医疗和法律任务中，在40%稀疏度下保持高达98%的密集模型性能，优于最先进方法。

Conclusion: 分析显示领域依赖的结构变化会降低通用剪枝掩码的有效性，强调需要针对每个领域量身定制的自适应、领域感知剪枝策略。
...
TL;DR: 本文提出了一个强化学习框架来优化磁共振指纹成像中的翻转角调度，通过非周期性模式增强指纹可分性，并可能减少重复时间以加速采集。


<details>
  <summary>Details</summary>
Motivation: 磁共振指纹成像利用可调采集参数产生的瞬态信号动态，但优化序列参数（如翻转角）是一个复杂的高维顺序决策问题。强化学习为自动参数选择提供了有前景的方法，以优化脉冲序列来最大化参数空间中指纹的可区分性。

Method: 引入强化学习框架来优化磁共振指纹成像中的翻转角调度，通过强化学习算法自动选择参数，生成非周期性模式的调度方案。

Result: 展示了学习到的调度方案表现出非周期性模式，增强了指纹的可分性。此外，强化学习优化的调度可能减少重复时间，从而潜在地加速磁共振指纹成像采集。

Conclusion: 强化学习框架能够有效优化磁共振指纹成像中的翻转角调度，生成非周期性模式以增强指纹可分性，并可能通过减少重复时间来加速采集过程。
...
TL;DR: 本文针对RL微调大语言模型导致的多样性崩溃问题，提出了理论分析和解决方案。通过证明RL微调存在选择和强化偏差导致多样性崩溃，作者提出了差分平滑方法，在保证正确性的同时提升多样性，在多个任务和模型规模上均优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 现有解决RL微调多样性崩溃的启发式方法存在随意性，经常在正确性和多样性之间权衡取舍，效果不稳定且有时相互矛盾。需要建立理论基础并提出更原则性的解决方案。

Method: 首先形式化证明RL微调导致多样性崩溃的机制（选择和强化偏差），然后提出差分平滑方法，该方法仅在正确轨迹上应用奖励修改来提升多样性。

Result: 在1B到7B参数的模型上，在CountDown和数学推理等多个领域进行实验，差分平滑方法在Pass@1和Pass@k指标上均优于传统RL方法和基于熵的启发式方法，在AIME24数据集上提升达6.7%。

Conclusion: 差分平滑方法在理论上优于现有启发式方法，能同时提升正确性和多样性，为RL微调中的多样性问题提供了原则性解决方案。
...
### [154] [Prompt Fairness: Sub-group Disparities in LLMs](https://arxiv.org/abs/2511.19956)
*Meiyu Zhong,Noel Teku,Ravi Tandon*

Main category: cs.LG

TL;DR: 本文研究了LLMs中的提示公平性问题，发现相同问题但不同措辞的提示会导致模型产生不同质量的回答。作者提出了信息论指标来量化这种偏差，并通过多数投票和提示中立化等干预措施来减少跨群体差异。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在不同用户/风格的提示下，即使询问相同问题，也可能产生质量差异显著的响应。这种提示公平性问题需要被量化和解决，以确保模型对不同用户群体的公平性。

Method: 使用信息论指标来捕捉偏差的两个维度：子群体敏感性和跨群体一致性。提出实际干预措施，包括跨多个生成的多数投票和提示中立化。

Result: 实验显示，在缓解前，跨群体分歧值达到0.28，通常在0.14-0.22范围内。应用中立化和多生成策略后，这些分歧持续减少，最大差距降至0.22，许多距离降至0.17或以下。

Conclusion: 通过提出的干预措施，可以有效减少LLMs对不同用户群体提示的敏感性差异，提高响应稳定性和跨用户群体的公平性。
...
### [155] [ParaBlock: Communication-Computation Parallel Block Coordinate Federated Learning for Large Language Models](https://arxiv.org/abs/2511.19959)
...
Result: 经验评估显示，ParaBlock在保持强大性能的同时，显著提高了通信效率。

Conclusion: ParaBlock成功解决了联邦学习大语言模型训练中的通信效率问题，同时保持了模型的收敛性能。
...
### [156] [Stragglers Can Contribute More: Uncertainty-Aware Distillation for Asynchronous Federated Learning](https://arxiv.org/abs/2511.19966)
...
<details>
  <summary>Details</summary>
Motivation: 异步联邦学习虽然提高了效率和可扩展性，但面临两个主要挑战：过时更新导致的模型性能下降，以及快速客户端在异构数据分布下主导学习过程带来的偏差。现有方法通常只解决其中一个问题，造成冲突。

Method: 提出FedEcho框架，采用不确定性感知蒸馏技术，使服务器能够评估滞后客户端预测的可靠性，并根据估计的不确定性动态调整这些预测的影响力。

Result: 通过大量实验证明，FedEcho在异步联邦学习基准测试中始终优于现有方法，实现了稳健的性能，且无需访问私有客户端数据。

Conclusion: FedEcho通过不确定性感知蒸馏有效缓解了过时更新和数据异构性的负面影响，在异步联邦学习中取得了显著性能提升。
...
TL;DR: CHONKNORIS是一种基于牛顿-康托罗维奇方法的神经算子学习范式，通过回归Tikhonov正则化牛顿更新的Cholesky因子，实现了机器精度的非线性PDE求解。该方法在多种前向和逆问题中表现出色，并提出了基础模型变体FONKNORIS用于求解未见过的非线性PDE。


<details>
  <summary>Details</summary>
Motivation: 现有神经算子学习方法在增加复杂度时往往无法显著提升精度，与更简单的核方法和降阶模型相当。本文旨在解决这一局限，开发能够达到机器精度的算子学习方法。

Method: CHONKNORIS方法不直接回归解算子，而是回归与Tikhonov正则化牛顿-康托罗维奇更新相关的椭圆算子的Cholesky因子。通过展开迭代获得神经架构，其机器精度行为源于实现收缩映射，相比端到端逼近解算子需要低得多的精度。
...
Conclusion: CHONKNORIS通过结合数值分析中的牛顿型方法，实现了机器精度的算子学习，在多种非线性前向和逆PDE问题中表现优异。该方法为神经算子学习提供了新的范式，并展示了基础模型在求解未见PDE方面的潜力。
...
### [158] [On-Demand Multi-Task Sparsity for Efficient Large-Model Deployment on Edge Devices](https://arxiv.org/abs/2511.19986)
...
<details>
  <summary>Details</summary>
Motivation: 现有稀疏优化方法针对单个任务独立优化，忽视了频繁任务切换时产生的显著I/O开销问题。

Method: 将权重分解为可重用的块粒度单元，跨任务对齐稀疏结构以最大化重叠，动态加载下一个任务所需的小差分块集。
...
### [159] [RankOOD - Class Ranking-based Out-of-Distribution Detection](https://arxiv.org/abs/2511.19996)
*Dishanika Denipitiyage,Naveen Karunanayake,Suranga Seneviratne,Sanjay Chawla*

Main category: cs.LG

TL;DR: RankOOD是一种基于排序的离群分布检测方法，通过使用Plackett-Luce损失训练模型，利用ID类预测的排序模式来识别OOD样本。


<details>
  <summary>Details</summary>
Motivation: 基于交叉熵损失训练的深度学习模型在ID类预测中会产生特定的排序模式，而OOD样本不太可能遵循这种排序分类模式。

Method: 首先使用初始分类器为每个类提取排序列表，然后使用Plackett-Luce损失进行第二轮训练，其中类排序作为预测变量。

Result: 在TinyImageNet近OOD评估基准上达到最先进性能，将FPR95降低了4.3%。

Conclusion: RankOOD通过利用类预测的排序模式，有效提升了OOD检测性能。
...
### [160] [REWA: Witness-Overlap Theory -- Foundations for Composable Binary Similarity Systems](https://arxiv.org/abs/2511.19998)
*Nikit Phadke*

Main category: cs.LG

TL;DR: REWA提出了一种基于见证重叠结构的通用相似性理论，将各种相似性定义统一为单调见证重叠，并证明可压缩编码为对数复杂度比特表示，同时保持top-k排序保证。


<details>
  <summary>Details</summary>
Motivation: 现有相似性系统（如Bloom过滤器、minhash、LSH等）各自为政，缺乏统一理论框架。REWA旨在构建一个通用的相似性理论，能够统一处理来自图结构、因果关系、时间结构、拓扑特征等多种来源的相似性。

Method: REWA系统包含三个组件：(1)有限见证集W(v)，(2)从每个见证生成的半随机比特分配，(3)期望相似性在重叠Δ(u,v)=|W(u)∩W(v)|中的单调性。通过重叠间隙条件保证使用m=O(log(|V|/δ))比特即可保持top-k排序。

Result: 证明了在见证集满足重叠间隙条件时，top-k排序可以通过对数复杂度的比特编码保持。该理论统一了Bloom过滤器、minhash、LSH位图、随机投影、草图等多种相似性技术。

Conclusion: REWA为相似性系统提供了原则性基础，其行为由见证重叠而非哈希函数工程决定。该框架支持模块化构建相似性系统，具有组合性设计空间，并可扩展到多比特编码、加权见证和非集合表示等扩展。
...
TL;DR: 研究表明，在零样本设置下，Sundial时间序列基础模型在输入上下文窗口足够长时（覆盖超过1-2个完整季节周期），能够超越完全训练的LSTM模型，首次证明通用基础模型无需任务特定调优即可在遥感时间序列预测中超越专业监督模型。


<details>
  <summary>Details</summary>
Motivation: 研究时间序列基础模型在农业监测中叶片面积指数预测的零样本能力，探索预训练模型在环境应用中的潜力。
...
Conclusion: 预训练时间序列基础模型在农业和环境应用中具有作为即插即用预测器的强大潜力，无需任务特定调优即可超越专业监督模型。
...
TL;DR: iRadioDiff是一种基于扩散模型的室内无线电地图构建框架，通过结合物理信息提示和多路径关键先验，实现了高保真度的无线电地图生成，在室内定位任务中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 传统电磁求解器构建室内无线电地图延迟过高，而基于学习的方法依赖于稀疏测量或同质材料假设，无法准确建模室内环境的异质性和多路径特性。

Method: 提出采样自由的扩散框架iRadioDiff，通过接入点位置和物理信息提示（材料反射和透射系数）进行条件生成，并整合衍射点、强透射边界和视线轮廓等多路径关键先验。

Result: 实验表明iRadioDiff在室内无线电地图构建和基于接收信号强度的室内定位任务中达到最先进性能，并具有良好的跨布局和材料配置泛化能力。
...
TL;DR: 提出DGF方案，通过双图滤波和三元交叉对比训练策略，解决多模态属性图中模态间相关性低和特征噪声强的问题，显著提升聚类性能。


<details>
  <summary>Details</summary>
Motivation: 现有多视图聚类方法依赖视图间高相关性，但忽略了多模态属性图中模态相关性低和特征噪声强的特点，导致聚类性能不佳。

Method: 提出双图滤波方案，包含特征级去噪组件和三元交叉对比训练策略，通过跨模态、邻域和社区的实例级对比学习学习鲁棒节点表示。

Result: 在8个基准MMAG数据集上的实验表明，DGF在聚类质量方面显著优于现有最先进方法。

Conclusion: DGF方案有效解决了多模态属性图中的独特挑战，为多模态图聚类提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有无监督方法在仅使用正常数据训练时，倾向于重构正常模式，导致在面对微弱异常前兆时预测被正常模式主导，淹没了预测所需的信号。

Method: 提出RED-F框架，包含重构消除模型(REM)和双流对比预测模型(DFM)。REM使用混合时频机制消除前兆生成纯净基线，DFM接收纯净基线和保留前兆的原始序列进行对比预测，通过计算两个预测流之间的差异来放大前兆信号。

Result: 在六个真实世界数据集上的广泛实验表明，RED-F在异常预测任务中表现出卓越能力。

Conclusion: RED-F通过重构消除和双流对比预测机制，成功地将困难的绝对信号检测任务转化为更简单、更鲁棒的相对轨迹比较任务，有效提升了微弱异常前兆的检测能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决基于模型的强化学习（MBRL）中未知系统动态下的高效探索挑战，需要从在线交互中直接学习。

Method: 学习不确定性感知的动态模型，贪婪地最大化外部奖励与智能体认知不确定性的加权和，兼容任何策略优化器或规划器。

Result: 在状态基和视觉控制环境中表现出色，在动态RC汽车硬件上优于最先进方法，展示了原理性探索对MBRL的益处。

Conclusion: SOMBRL为MBRL提供了灵活、可扩展的原理性探索解决方案，在非线性动态的各种设置下具有次线性遗憾性能。
...
TL;DR: 本文提出CRUX-V模型，通过结构化中间表示CRUX来弥合自由形式自然语言描述与精确Verilog代码生成之间的差距，在多个Verilog生成基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于自由形式自然语言描述的硬件描述语言生成方法存在模糊、冗余和非结构化问题，给下游Verilog代码生成带来挑战。

Method: 引入结构化中间空间CRUX捕获用户意图的语义本质，并设计包含联合表达建模和双空间优化的两阶段训练框架。
...
### [167] [Multivariate Forecasting of Bitcoin Volatility with Gradient Boosting: Deterministic, Probabilistic, and Feature Importance Perspectives](https://arxiv.org/abs/2511.20105)
...
Method: 使用69个预测因子构建LightGBM模型，采用两种分位数方法进行概率预测：基于pinball损失函数的直接分位数回归，以及将点预测转换为预测分布的残差模拟方法。使用基于增益和排列的特征重要性技术识别主要波动驱动因素。

Result: 研究结果表明，LightGBM模型在捕捉加密货币市场非线性特征方面表现有效，特征重要性分析一致显示交易量、滞后波动率指标、投资者关注度和市值是主要波动驱动因素。

Conclusion: LightGBM模型能够有效预测比特币波动率，同时提供对波动动态的可解释性洞察，交易量、滞后波动率、投资者关注度和市值是识别出的关键驱动因素。
...
### [168] [On the Limits of Momentum in Decentralized and Federated Optimization](https://arxiv.org/abs/2511.20168)
*Riccardo Zaccone,Sai Praneeth Karimireddy,Carlo Masone*

Main category: cs.LG

TL;DR: 该论文分析了联邦学习中动量方法在循环客户端参与下的表现，理论证明动量仍然受到统计异质性的影响，且与SGD类似，任何快于Θ(1/t)的步长衰减都会导致收敛到依赖于初始化和异质性边界的常数值。


<details>
  <summary>Details</summary>
Motivation: 探索动量方法在联邦学习(FL)中是否能缓解统计异质性的影响，特别是在去中心化场景下只有部分工作节点参与每轮训练的情况。

Method: 理论分析动量方法在循环客户端参与下的收敛性，并与SGD进行对比，通过数值实验和深度学习实验验证理论结果。

Result: 动量方法在统计异质性下无法保证收敛，与SGD类似，步长衰减快于Θ(1/t)会导致收敛到依赖于初始化和异质性边界的常数值。

Conclusion: 动量方法在联邦学习中并不能完全解决统计异质性问题，其收敛行为与SGD相似，都受到异质性的限制。
...
### [169] [Learning Subgroups with Maximum Treatment Effects without Causal Heuristics](https://arxiv.org/abs/2511.20189)
*Lincen Yang,Zhong Li,Matthijs van Leeuwen,Saber Salehkaleybar*

Main category: cs.LG

TL;DR: 本文提出了一种在结构因果模型框架下发现最大平均治疗效果亚组的新方法，避免了传统方法中依赖点估计或临时因果启发式的问题。


<details>
  <summary>Details</summary>
Motivation: 现有亚组发现方法存在两个主要问题：一是依赖点估计条件治疗效果，将亚组发现问题转化为更难的准确点估计问题；二是使用缺乏严格理论依据的临时因果启发式方法。

Method: 在基于分区的模型假设下，将最优亚组发现问题简化为恢复数据生成模型的标准监督学习问题（回归或分类），并采用CART等基于分区的方法直接从数据中学习亚组。

Result: 在大量合成和半合成数据集上的实验表明，该方法比多种基线方法更准确地识别出具有最大治疗效果的亚组。

Conclusion: 在结构因果模型框架下，亚组发现问题可以转化为标准监督学习问题，避免了因果启发式方法，能够更准确地识别最优亚组。
...
### [170] [In-Context Compositional Learning via Sparse Coding Transformer](https://arxiv.org/abs/2511.20194)
...
<details>
  <summary>Details</summary>
Motivation: Transformer架构在处理组合学习任务时面临挑战，因为它们不是专门为处理组合任务设计的，且提供的结构归纳偏差有限。本文旨在增强Transformer在组合任务中的能力。

Method: 将注意力块重新解释为通过投影到两组学习字典原子的映射：编码字典将输入分解为一组系数，表示输入的组合结构；解码字典使用稀疏系数线性组合生成输出。此外，提出通过上下文示例的系数线性组合来估计目标问题的系数。

Result: 在S-RAVEN和RAVEN数据集上验证了方法的有效性。对于某些组合泛化任务，该方法在标准Transformer失败时仍能保持性能。

Conclusion: 基于稀疏编码的注意力改进方法能够有效学习和应用组合规则，增强Transformer在组合任务中的能力。
...
### [171] [Communication-Efficient Learning for Satellite Constellations](https://arxiv.org/abs/2511.20220)
*Ruxandra-Stefania Tudose,Moritz H. W. Grüss,Grace Ra Kim,Karl H. Johansson,Nicola Bastianello*

Main category: cs.LG

TL;DR: 该论文提出了一种用于低地球轨道卫星星座的联邦学习算法，通过本地训练、压缩和误差反馈机制来提高通信效率，同时保持模型准确性。
...
Method: 采用联邦学习方法，卫星收集并本地处理数据，地面站聚合本地模型。使用本地训练减少通信次数，压缩减少通信量，并引入误差反馈机制提高准确性。

Result: 通过理论分析和在真实空间场景下的仿真，证明所提算法相比现有技术具有更优越的性能。

Conclusion: 提出的算法在通信效率和模型准确性方面都表现出色，误差反馈机制还可广泛应用于其他算法。
...
### [172] [Interpretable Air Pollution Forecasting by Physics-Guided Spatiotemporal Decoupling](https://arxiv.org/abs/2511.20257)
*Zhiguo Zhang,Xiaoliang Ma,Daniel Schlesinger*

Main category: cs.LG

TL;DR: 提出了一种物理引导、可解释的时空学习框架，用于空气污染预测，通过分解时空行为为两个透明模块，在性能和解译性之间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 准确的空气污染预测对公共健康至关重要，但大多数模型在性能和解译性之间存在权衡，需要开发既准确又可解释的预测模型。

Method: 将污染物浓度的时空行为分解为两个透明模块：物理引导的传输核（考虑风和地理条件的对流）和可解释的注意力机制（学习局部响应并归因于历史滞后和外部驱动因素）。
...
### [173] [Beyond Components: Singular Vector-Based Interpretability of Transformer Circuits](https://arxiv.org/abs/2511.20273)
*Areeb Ahmad,Abhinav Joshi,Ashutosh Modi*

Main category: cs.LG

TL;DR: 本文提出了一种更细粒度的视角，将Transformer中的注意力头和MLP层分解为正交奇异方向，揭示了单个组件内叠加的独立计算。在标准任务上的验证表明，先前识别的功能头实际上编码了多个重叠的子功能。


<details>
  <summary>Details</summary>
Motivation: 现有的机制可解释性方法通常将注意力头和MLP层视为不可分割的单元，忽视了它们内部可能学习到的功能子结构。本文旨在揭示Transformer组件内部更细粒度的计算结构。

Method: 将Transformer组件（注意力头和MLP层）分解为正交奇异方向，分析这些方向上的计算模式。在标准任务（IOI、GP、GT）上验证该方法。

Result: 发现先前识别的典型功能头（如名称移动器）编码了多个与不同奇异方向对齐的重叠子功能。计算图中的节点在特定低秩方向上表现出强激活，表明有意义的计算驻留在紧凑子空间中。

Conclusion: Transformer计算比先前假设的更加分布式、结构化和组合化。这种视角为细粒度机制可解释性和模型内部理解开辟了新途径。
...
### [174] [HVAdam: A Full-Dimension Adaptive Optimizer](https://arxiv.org/abs/2511.20277)
*Yiheng Zhang,Shaowu Wu,Yuanzhuo Xu,Jiajun Wu,Shang Xu,Steve Drew,Xiaoguang Niu*

Main category: cs.LG

TL;DR: Anon是一种新型优化器，通过可调节的自适应性和增量延迟更新机制，能够在SGD和Adam之间灵活切换并超越两者性能，在各种任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 自适应优化器如Adam在大规模模型训练中表现优异，但在经典架构上泛化能力不如非自适应方法如SGD。研究发现自适应预调节器限制了优化器适应不同优化景观的能力。

Method: 提出Anon优化器，具有连续可调的自适应性，可在SGD和Adam行为之间插值甚至外推。引入增量延迟更新(IDU)机制，比AMSGrad的硬最大跟踪策略更灵活，增强对梯度噪声的鲁棒性。

Result: 理论上在凸和非凸设置下建立了收敛保证。实证中，Anon在图像分类、扩散模型和语言建模任务上持续优于最先进的优化器。

Conclusion: 自适应性可作为有价值的设计原则，Anon提供了首个统一可靠的框架，能够弥合经典和现代优化器之间的差距并超越其优势特性。
...
### [175] [Geometry of Decision Making in Language Models](https://arxiv.org/abs/2511.20315)
*Abhinav Joshi,Divyanshu Bhatt,Ashutosh Modi*

Main category: cs.LG

TL;DR: 本文通过内在维度(Intrinsic Dimension)分析大型语言模型在多项选择题任务中的决策过程，发现模型在早期层使用低维流形，中间层扩展维度，后期层再次压缩到与决策相关的表示。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多样化任务中表现出强大的泛化能力，但其内部决策过程仍不透明，需要从几何角度理解隐藏表示的结构。

Method: 使用28个开源Transformer模型，通过多个估计器估计各层的内在维度，并量化每层在多项选择题任务上的性能。

Result: 发现模型存在一致的ID模式：早期层在低维流形上操作，中间层扩展空间，后期层再次压缩并收敛到与决策相关的表示。

Conclusion: LLMs隐式学习将语言输入投影到结构化、低维的流形上，这些流形与任务特定决策对齐，为理解语言模型中泛化和推理的出现提供了新的几何视角。
...
### [176] [MXtalTools: A Toolkit for Machine Learning on Molecular Crystals](https://arxiv.org/abs/2511.20327)
*Michael Kilgour,Mark E. Tuckerman,Jutta Rogal*

Main category: cs.LG

TL;DR: MXtalTools是一个用于分子晶体数据驱动建模的灵活Python包，支持机器学习研究，包含数据集合成、模型训练、晶体参数化、结构采样和优化等功能。


<details>
  <summary>Details</summary>
Motivation: 为分子固态的机器学习研究提供灵活的工具包，促进数据驱动的分子晶体建模。

Method: 开发包含多个功能模块的Python包：数据集合成与整理、集成工作流、晶体参数化与表示、结构采样与优化、端到端可微分晶体处理。

Result: 成功开发了模块化的MXtalTools包，支持CUDA加速的高通量晶体建模，代码开源并提供详细文档。
...
### [177] [Soft Adaptive Policy Optimization](https://arxiv.org/abs/2511.20347)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于分组的策略优化方法（如GSPO和GRPO）使用硬裁剪处理令牌级重要性比率的高方差问题，难以同时保持稳定性和有效学习，特别是在专家混合模型中问题更加严重。

Method: 提出软自适应策略优化（SAPO），用平滑的温度控制门替代硬裁剪，自适应地衰减离策略更新，同时保留有用的学习信号。该方法既保持序列级一致性，又实现令牌级自适应。

Result: 在数学推理基准测试中，SAPO在相同训练预算下表现出更好的训练稳定性和更高的Pass@1性能。使用SAPO训练的Qwen3-VL模型系列在不同任务和模型大小上都获得了持续的性能提升。
...
### [178] [Identifying environmental factors associated with tetrodotoxin contamination in bivalve mollusks using eXplainable AI](https://arxiv.org/abs/2511.20395)
...
<details>
  <summary>Details</summary>
Motivation: 自2012年以来，欧洲温带水域的双壳类软体动物中发现河豚毒素污染，这带来了食品安全风险和经济损失，需要早期预测来保障食品工业和监管机构。

Method: 开发了基于深度学习的可解释模型，输入气象和水文特征，输出河豚毒素污染的存在或缺失。

Result: 结果显示日出时间、日落时间、全球辐射、水温和氯化物浓度对河豚毒素污染贡献最大，有效日照小时数是重要驱动因素。

Conclusion: 可解释的深度学习模型识别出日照小时数、全球辐射、水温和水氯化物浓度与河豚毒素污染相关，该方法对减轻海洋毒素风险具有价值。
...
### [179] [Short-Range Oversquashing](https://arxiv.org/abs/2511.20406)
*Yaaqov Mishayev,Yonatan Sverdlov,Tal Amir,Nadav Dym*

Main category: cs.LG

TL;DR: 本文揭示了MPNN中的过度压缩现象不仅存在于长距离任务中，也存在于短距离任务中，并区分了瓶颈现象和梯度消失两种机制。研究表明虚拟节点无法解决短距离瓶颈，而Transformer是更有效的解决方案。


<details>
  <summary>Details</summary>
Motivation: MPNN在图学习中广泛应用，但其处理长距离信息的能力受到过度压缩现象的限制。现有研究对Graph Transformers和MPNN改进方案存在争议，需要更深入地理解过度压缩的本质。

Method: 通过分析短距离任务中的过度压缩现象，区分瓶颈现象和梯度消失两种机制，并比较虚拟节点和Transformer在解决这些问题上的效果。

Result: 发现短距离任务中也存在过度压缩，且现有解释无法捕捉短距离瓶颈效应。虚拟节点无法解决短距离瓶颈，而Transformer能够成功处理此类任务。

Conclusion: Transformer相比专门的MPNN是解决过度压缩问题的更有效方案，特别是在处理短距离瓶颈现象方面。
...
TL;DR: 本文证明了投票分类器的首个基于间隔的泛化界，该界限在假设集大小、间隔、具有给定间隔的训练点比例、训练样本数量和失败概率之间的权衡方面是渐近紧的。


<details>
  <summary>Details</summary>
Motivation: 为投票分类器建立更精确的泛化理论保证，填补现有理论在权衡关系方面的不足。

Method: 通过数学证明方法，构建基于间隔的泛化边界分析框架。

Result: 获得了在多个关键参数（假设集大小、间隔、训练点比例等）权衡下渐近紧的泛化界。

Conclusion: 该结果为投票分类器的泛化性能提供了更精确的理论保证，在理论分析和实际应用中具有重要意义。
...
### [181] [Diffusion for Fusion: Designing Stellarators with Generative AI](https://arxiv.org/abs/2511.20445)
*Misha Padidar,Teresa Huang,Andrew Giuliani,Marina Spivak*

Main category: cs.LG

TL;DR: 本文提出了一个机器学习的开放逆向问题：快速生成具有理想特性的高质量仿星器设计。通过条件扩散模型在QUASR数据库上训练，生成具有特定方面比和平均旋转变换的准对称仿星器设计。


<details>
  <summary>Details</summary>
Motivation: 仿星器设计通常需要数小时在计算集群上解决，开发快速设计方法对推进聚变研究至关重要。鉴于最近优化仿星器大型数据集的发展，机器学习方法成为潜在候选方案。

Method: 使用条件扩散模型在QUASR数据库上进行训练，生成具有目标特性（方面比和平均旋转变换）的准对称仿星器设计，并将模型应用于训练期间未见过的特性设计。

Result: 生成的许多仿星器表现出良好性能：准对称性和目标特性的偏差小于5%。适度的准对称性偏差表明有机会达到低于1%的目标。

Conclusion: 生成建模在推进仿星器设计方面具有多个有前景的应用方向，为机器学习社区提供了一个重要的开放逆向问题。
...
### [182] [Towards Trustworthy Wi-Fi Sensing: Systematic Evaluation of Deep Learning Model Robustness to Adversarial Attacks](https://arxiv.org/abs/2511.20456)
...
<details>
  <summary>Details</summary>
Motivation: 随着机器学习在基于CSI的人类感知系统中日益重要，这些系统依赖的模型决策易受细微扰动影响，在泛在感知应用中引发安全和可靠性担忧。在无线感知实际部署前，量化理解模型鲁棒性至关重要。

Method: 建立了评估框架，比较紧凑时间自编码器模型与大型深度架构在三个公共数据集上的表现，量化模型规模、训练机制和物理约束对鲁棒性的影响，测试了白盒、黑盒/迁移和通用扰动等不同威胁模型。

Result: 实验显示较小模型在干净数据上性能相当但鲁棒性明显较差；物理可实现信号空间扰动相比无约束特征空间攻击显著降低了攻击成功率；对抗训练能提高平均鲁棒准确率，仅适度降低干净性能。

Conclusion: 这些发现为鲁棒性估计提供了量化基准，并为安全可信的人类中心感知系统设计提供了指导原则，有助于无线感知向可靠跨域操作发展。
...
### [183] [NVIDIA Nemotron Parse 1.1](https://arxiv.org/abs/2511.20478)
...
TL;DR: Nemotron-Parse-1.1是一个轻量级文档解析和OCR模型，相比前代提升了OCR、Markdown格式化、表格解析和图像文本提取能力，支持更长输出序列，采用885M参数编码器-解码器架构，在公开基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 提升文档解析和OCR能力，特别是在通用OCR、Markdown格式化、结构化表格解析以及图片、图表和示意图中的文本提取方面，同时支持处理视觉密集文档。

Method: 采用编码器-解码器架构，包含885M参数，其中语言解码器为256M参数，提取文本段的边界框和对应的语义类别。

Result: 在公开基准测试中达到有竞争力的准确率，成为强大的轻量级OCR解决方案，同时发布了优化版本Nemotron-Parse-1.1-TC，在视觉token长度减少的情况下提供20%速度提升且质量损失最小。

Conclusion: Nemotron-Parse-1.1是一个高效的轻量级文档解析模型，在多个文档处理任务上表现优异，并通过公开发布模型权重、优化容器和训练数据子集来促进社区使用。
...
### [184] [MTBBench: A Multimodal Sequential Clinical Decision-Making Benchmark in Oncology](https://arxiv.org/abs/2511.20490)
...
TL;DR: MTBBench是一个模拟分子肿瘤委员会决策过程的基准测试，针对多模态大语言模型在生物医学推理中的局限性，特别是处理多模态、纵向临床数据的能力不足问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试无法捕捉真实世界临床工作流程的复杂性，特别是分子肿瘤委员会这种需要整合异构数据和随时间演变的洞察的多智能体决策环境。

Method: 开发MTBBench基准测试，模拟MTB风格的决策过程，包含临床挑战性的多模态和纵向肿瘤学问题，并通过临床医生验证的真实标注。

Result: 基准测试显示，即使是规模较大的LLM也缺乏可靠性，经常产生幻觉，难以从时间分辨数据中推理，无法调和冲突证据或不同模态。

Conclusion: MTBBench提供了一个具有挑战性和现实性的测试平台，通过基于基础模型的工具增强多模态和纵向推理，在精准肿瘤学的MTB环境中推进多模态LLM的推理、可靠性和工具使用能力。
...
TL;DR: 提出DP-MicroAdam，一种内存高效且支持稀疏性的自适应差分隐私优化器，在多个基准测试中优于现有自适应DP优化器，并与DP-SGD竞争或超越其性能。
...
Method: 开发DP-MicroAdam优化器，具有内存效率和稀疏性感知特性，在随机非凸优化中证明其以最优速率收敛。
...
TL;DR: 本文对Adam优化器中的偏差校正功能进行了系统性研究，发现在最优超参数配置下，偏差校正不会改善最终测试性能，且在没有适当学习率调度时可能损害性能。
...
Method: 通过在视觉和语言建模任务上进行一系列系统性消融实验，研究偏差校正在不同超参数配置下的表现。

Result: 研究表明，在最优超参数配置下，包含偏差校正不会提高最终测试性能。除非实施适当的学习率调度，否则偏差校正有时会对性能产生负面影响。偏差校正可被重新解释为一种隐式学习率调度，其行为强烈依赖于平滑超参数β₁、β₂的选择。

Conclusion: 研究结果挑战了偏差校正组件被普遍包含的做法，表明其并非在所有情况下都是必要的。
...
TL;DR: UFNO-FiLM改进了UFNO架构，通过FiLM层解耦标量输入和空间特征，并使用空间加权损失函数，在次表面多相流预测中将气体饱和度MAE降低了21%。


<details>
  <summary>Details</summary>
Motivation: UFNO虽然通过并行UNet路径提升了预测精度，但低效地将标量输入作为空间分布场处理，导致频域中处理冗余常数信号，且标准损失函数未考虑误差敏感性的空间变化。

Method: 引入FiLM层解耦标量输入与空间特征，避免常数信号进入傅里叶变换；采用空间加权损失函数优先学习关键区域。

Result: 在次表面多相流实验中，相比UFNO实现了21%的气体饱和度MAE降低。

Conclusion: UFNO-FiLM通过FiLM层和空间加权损失有效提升了预测精度，特别是在物理重要性高的区域。
...
<details>
  <summary>Details</summary>
Motivation: 传统工业部署采用两阶段流水线（GNN预训练生成节点嵌入，然后用于下游推荐系统），存在计算开销大和缺乏联合优化的局限性。

Method: 包含三个关键组件：高效子图采样、图特征自编码器作为辅助自监督任务、两级特征融合机制结合基于Gradnorm的动态损失平衡。

Result: 离线评估和在线A/B测试显示，在停留时长上相对提升0.133%，用户跳过视频数量减少0.3171%，在多个推荐指标上显著优于传统方法。
...
### [189] [A Tale of Two Geometries: Adaptive Optimizers and Non-Euclidean Descent](https://arxiv.org/abs/2511.20584)
*Shuo Xie,Tianhao Wang,Beining Wu,Zhiyuan Li*

Main category: cs.LG

TL;DR: 该论文揭示了自适应优化器与归一化最速下降(NSD)之间的紧密联系，通过引入自适应平滑性概念统一分析框架，并在凸和非凸设置下建立了收敛理论，同时证明了自适应平滑性能够实现Nesterov动量加速。
...
Method: 扩展自适应平滑性理论到非凸设置，引入自适应梯度方差概念，建立自适应优化器的收敛分析框架，并与标准平滑性条件进行对比。

Result: 证明了自适应平滑性精确刻画了自适应优化器的收敛特性，在凸设置下能够实现Nesterov动量加速，且在非欧几里得几何中获得了标准平滑性无法达到的维度无关收敛保证。

Conclusion: 自适应平滑性为理解自适应优化器提供了统一的理论框架，揭示了其在非欧几里得几何中的独特优势，为优化算法设计提供了新的理论指导。
...
### [190] [Anatomica: Localized Control over Geometric and Topological Properties for Anatomical Diffusion Models](https://arxiv.org/abs/2511.20587)
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够灵活控制解剖结构几何和拓扑特征的框架，用于合成数据集的合理设计，以支持虚拟试验或机器学习工作流程。
...
### [191] [Attention Trajectories as a Diagnostic Axis for Deep Reinforcement Learning](https://arxiv.org/abs/2511.20591)
*Charlotte Beylier,Hannah Selder,Arthur Fleig,Simon M. Hofmann,Nico Scherf*

Main category: cs.LG

TL;DR: 本文提出了注意力导向指标（ATOMs）来研究强化学习智能体在训练过程中注意力的发展，通过在Pong游戏变体上的实验验证了ATOMs能够区分不同游戏变体训练出的智能体的注意力模式，并发现这些注意力差异转化为行为差异。


<details>
  <summary>Details</summary>
Motivation: 强化学习智能体的学习过程除了其学习算法的数学公式外，仍然缺乏深入理解。为了填补这一空白，需要研究RL智能体在训练过程中注意力的发展。

Method: 引入注意力导向指标（ATOMs），在三个设计用于教授不同行为的Pong游戏变体上进行受控实验，并辅以行为评估。

Result: ATOMs成功描绘了在每个游戏变体上训练的智能体的注意力模式，这些注意力模式的差异转化为智能体行为的差异。通过训练期间持续监测ATOMs，观察到智能体的注意力分阶段发展，且这些阶段在不同游戏变体中保持一致。
...
### [192] [Latent Diffusion Inversion Requires Understanding the Latent Space](https://arxiv.org/abs/2511.20592)
*Mingxing Rao,Bowen Qu,Daniel Moyer*

Main category: cs.LG

TL;DR: 本文研究了潜在扩散模型中的记忆化现象，发现扩散模型在潜在代码上表现出不均匀的记忆化，倾向于过拟合位于解码器回拉度量高失真区域的样本。作者提出了一种基于解码器回拉度量的维度排序方法，识别对记忆化贡献最大的潜在维度，并证明在成员推理攻击中移除较少记忆化维度能显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有模型反转技术主要关注数据域的扩散模型，而忽略了潜在空间生成模型（如潜在扩散模型）中的编码器/解码器对和相应潜在代码的作用。本文旨在研究潜在扩散模型中的记忆化现象及其隐私风险。

Method: 提出了一种原则性方法来按维度对解码器回拉度量的贡献进行排序，识别对记忆化最负责的潜在维度。在基于分数的成员推理攻击中，移除较少记忆化维度来计算攻击统计量。

Result: 实验表明，移除较少记忆化维度后，成员推理攻击性能显著提升，在CIFAR-10、CelebA、ImageNet-1K等数据集上平均AUROC增益达2.7%，TPR@1%FPR大幅增加6.42%。
...
### [193] [The Driver-Blindness Phenomenon: Why Deep Sequence Models Default to Autocorrelation in Blood Glucose Forecasting](https://arxiv.org/abs/2511.20601)
*Heman Shakeri*

Main category: cs.LG

TL;DR: 该论文指出深度序列模型在血糖预测中存在"驱动因素盲区"问题，即无法有效利用胰岛素、饮食和活动等临床信息驱动因素，并提出了Δdrivers指标来量化这一问题。


<details>
  <summary>Details</summary>
Motivation: 当前深度序列模型在血糖预测中未能充分利用临床驱动因素，尽管这些因素的生理机制已被充分理解，这限制了模型的预测性能。

Method: 通过分析文献中Δdrivers指标（多变量模型相对于匹配单变量基线的性能增益），识别了导致驱动因素盲区的三个因素：架构偏向自相关、数据保真度差距和生理异质性，并提出了生理特征编码器、因果正则化和个性化等缓解策略。

Result: 研究发现文献中Δdrivers通常接近零，表明多变量模型未能有效利用驱动因素，驱动因素盲区普遍存在。

Conclusion: 建议未来工作常规报告Δdrivers指标，以防止驱动因素盲区模型被误认为是最先进的，并推荐采用生理特征编码、因果正则化和个性化等策略来缓解这一问题。
...
### [194] [DiFR: Inference Verification Despite Nondeterminism](https://arxiv.org/abs/2511.20621)
*Adam Karvonen,Daniel Reuter,Roy Rinberg,Luke Marks,Adrià Garriga-Alonso,Keri Warr*

Main category: cs.LG

TL;DR: Token-DiFR是一种通过比较生成令牌与可信参考实现预测来验证LLM推理输出的方法，使用采样种子同步约束有效输出，实现零成本正确性审计。


<details>
  <summary>Details</summary>
Motivation: 随着LLM推理需求增长，需要验证推理过程是否正确执行且未被篡改，但由于数值噪声导致重复运行结果不同，难以区分合法变异与实际问题。

Method: 引入Token-DiFR方法，通过采样种子同步将生成令牌与可信参考实现预测进行比较；同时提出Activation-DiFR，使用随机正交投影压缩激活值为紧凑指纹进行验证。

Result: Token-DiFR可靠识别采样错误、模拟错误和模型量化，在300个输出令牌内检测4位量化的AUC>0.999；Activation-DiFR仅需2个输出令牌即可检测4位量化，通信开销比现有方法减少25-75%。

Conclusion: 提出的方法能够有效验证LLM推理正确性，Token-DiFR提供零成本审计证据，Activation-DiFR实现高效前向传递验证，已开源集成vLLM加速实际部署。
...
### [195] [ROOT: Robust Orthogonalized Optimizer for Neural Network Training](https://arxiv.org/abs/2511.20626)
*Wei He,Kai Han,Hang Zhou,Hanting Chen,Zhicheng Liu,Xinghao Chen,Yunhe Wang*

Main category: cs.LG

TL;DR: ROOT是一种鲁棒正交化优化器，通过双重鲁棒机制增强训练稳定性：维度鲁棒的正交化方案和优化鲁棒框架，在噪声和非凸场景下相比Muon和Adam优化器实现更快的收敛和更优的最终性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型优化面临关键挑战，模型规模扩大加剧了对算法不精确性和训练不稳定性的敏感性。现有优化器在正交化精度和异常值诱导噪声方面存在鲁棒性限制。

Method: 开发维度鲁棒的正交化方案（使用自适应牛顿迭代和细粒度系数），以及通过近端优化的优化鲁棒框架来抑制异常值噪声同时保留有意义的梯度方向。

Result: 大量实验表明ROOT实现了显著改进的鲁棒性，相比Muon和Adam优化器具有更快的收敛速度和更优的最终性能，特别是在噪声和非凸场景下。
...
### [196] [Image2Gcode: Image-to-G-code Generation for Additive Manufacturing Using Diffusion-Transformer Model](https://arxiv.org/abs/2511.20636)
...
Result: 通过直接从2D图像生成结构化G代码，Image2Gcode消除了对CAD或STL中间件的需求，降低了增材制造的门槛，加速了从设计到制造的周期。

Conclusion: 这种方法支持从简单草图或视觉参考进行按需原型制作，并与上游2D到3D重建模块集成，实现了从概念到物理工件的自动化流程，是一个灵活、计算效率高的框架，提升了设计迭代、修复工作流程和分布式制造的可访问性。
